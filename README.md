...
```
  ...
  This repository is currently a local workspace, not a published global npm CLI. `pnpm install:cli` writes a managed source-checkout wrapper to `%APPDATA%\npm\nullius.cmd` on Windows or `~/.local/bin/nullius` on POSIX, refuses to replace an unmanaged command unless `--force` is explicit, and reports when the destination is not on `PATH`. A project-local launcher is also written during `nullius init`, so initialized research folders can continue with `./.nullius/bin/nullius status --json` on POSIX or `.\.nullius\bin\nullius.cmd status --json` on native Windows even when the global wrapper is absent.
