...
- **Standalone closeout**: `NEW-RT-04` ✅（2026-03-17，执行模型 `gpt-5-codex`；在 2026-03-15 bounded repair 纠偏基础上，最终收口不是停在 `executeDelegatedAgentRuntime()` library export，而是落到真实 shared tool surface `orch_run_execute_agent`：`packages/orchestrator/src/orch-tools/index.ts` 将其注册进 `ORCH_TOOL_SPECS`，`packages/orchestrator/src/orch-tools/agent-runtime.ts` 通过 `executeDelegatedAgentRuntime()` 真正消费 `AgentRunner` + `RunManifestManager`，`packages/hep-mcp/src/tools/registry/projectExtensions.ts` 与 `packages/hep-mcp/src/tools/dispatcher.ts` 则提供 host `createMessage` + loopback `callTool`，使 `resume_from` / `last_completed_step` 通过 production `handleToolCall(...)` 路径改变实际执行行为。新增 host-path contract `packages/hep-mcp/tests/contracts/orchRunExecuteAgent.test.ts` 锁定 manifest persist + crash/re-entry skip path；scoped acceptance 通过：`pnpm --filter @autoresearch/shared build`、`git diff --check`、`pnpm --filter @autoresearch/orchestrator test -- tests/run-manifest.test.ts tests/agent-runner-manifest.test.ts tests/research-loop-delegated-agent-runtime.test.ts tests/research-loop-runtime.test.ts tests/research-loop-smoke.test.ts`、`pnpm --filter @autoresearch/orchestrator build`、`pnpm --filter @autoresearch/zotero-mcp build`、`pnpm --filter @autoresearch/pdg-mcp build`、`pnpm --filter @autoresearch/arxiv-mcp build`、`pnpm --filter @autoresearch/openalex-mcp build`、`pnpm --filter @autoresearch/hepdata-mcp build`、`pnpm --filter @autoresearch/hep-mcp test -- tests/contracts/orchRunExecuteAgent.test.ts tests/contracts/orchRunApprove.test.ts`、`pnpm --filter @autoresearch/hep-mcp build`。formal review-swarm R1 暴露 dead-helper blocker；R2 由 `Opus + Gemini-3.1-Pro-Preview + OpenCode(kimi-for-coding/k2p5)` 收敛为 0 blocking（`CONVERGED_WITH_AMENDMENTS` / `CONVERGED` / `CONVERGED`），self-review 0 blocking。GitNexus pre/post `npx gitnexus analyze --force` 成功，但 MCP `context` / `detect_changes` 仍 `Transport closed`，因此 exact verification 继续以 direct source inspection + passing acceptance 为准。本批未扩到 `NEW-05a`、`NEW-07`、`EVO-13` 或 provider-local runtime wrapper；无新增稳定架构不变量，`.serena/memories/architecture-decisions.md` 不更新）
...
This project is indexed by GitNexus as **autoresearch-lab-new-rt04-prompt** (8490 symbols, 22802 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/autoresearch-lab-new-rt04-prompt/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/autoresearch-lab-new-rt04-prompt/context` | Codebase overview, check index freshness |
| `gitnexus://repo/autoresearch-lab-new-rt04-prompt/clusters` | All functional areas |
| `gitnexus://repo/autoresearch-lab-new-rt04-prompt/processes` | All execution flows |
| `gitnexus://repo/autoresearch-lab-new-rt04-prompt/process/{name}` | Step-by-step execution trace |
