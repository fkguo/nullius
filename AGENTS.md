...
- **Standalone pending**: `NEW-05a-hep-semantic-authority-deep-cleanup`（深度审计已落地到 `meta/docs/2026-03-10-hep-semantic-authority-deep-audit.md`，program prompt 已落地到 `meta/docs/prompts/prompt-2026-03-10-hep-semantic-deep-cleanup.md`；Batch A 已完成并并入 `NEW-05a-idea-core-domain-boundary` closeout，当前仅剩 Batch B-F 未完成。后续 pending scope 仅限 `hep-mcp` review/paper/question/assumption/conflict/grouping/challenge 等 active closed-lexicon authority 清理，以及仅在 provider-neutral rewrite 之后才允许考虑的 generic uplift；不得把本批 closeout 扩展成 Batch B-F 或 batch3）
...
This project is indexed by GitNexus as **autoresearch-lab** (9833 symbols, 22916 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
