...
- **完成态门禁**：只有当验收命令通过、`review-swarm` 收敛且三审 `blocking_issues = 0`、`self-review` 通过、tracker / memory / `AGENTS.md` 已同步，并且 `meta/REDESIGN_PLAN.md` 已按本轮实际结果完成同步或明确记录“已检查且无需更新”的理由后，实施项才可标记 `done`。
- **`REDESIGN_PLAN.md` 进度同步是显式硬门禁**：只要本轮 closeout 使 `meta/REDESIGN_PLAN.md` 中任一已有 item / batch / lane 的进度复选框、batch 分层表、完成态标题、当前现状描述、验收勾选或 closeout 叙事变得过时，就必须在同轮同步修正；不得以“phase 边界没变”或“tracker / prompt 已记录”作为跳过理由。只有在已逐段检查相关 plan 区域并确认不会产生进度漂移时，才允许写明“无需更新 REDESIGN_PLAN”。
...
- 若该条目或其 batch / lane 已在 `meta/REDESIGN_PLAN.md` 中出现进度复选框、batch 表、完成态标题、当前现状或验收勾选，则同轮必须同步检查并更新 `meta/REDESIGN_PLAN.md`；若无变更，也必须在 closeout 说明里显式写明“已检查，REDESIGN_PLAN 无需更新”。
...
This project is indexed by GitNexus as **autoresearch-lab** (11938 symbols, 26401 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
