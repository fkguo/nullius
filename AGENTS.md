...
- **Phase 0**: 14/14 完成 ✅
- **Phase 1**: 23/23 完成 ✅（`NEW-R03b` 现已同步为 `done`：此前 remaining gap 仅是“实现与 enhanced rereview 已完成，但当时未获 commit authorization，故暂未改 tracker 状态”。相关修订现已进入 `main`（至少包含 `90431f9` 与 `dafbfca`），并在当前 baseline 复核通过：`uv run --project packages/hep-autoresearch pytest packages/hep-autoresearch/tests -q` = `204 passed`，`uv run --project packages/hep-autoresearch pytest packages/hep-autoresearch/tests/test_scaffold_template_sync.py -q` = `3 passed`。formal review / enhanced rereview / self-review 先前已收敛为 0 blocking；GitNexus follow-up 的 `impact` / `context` 仍会间歇 `Transport closed`，因此完成态继续以 direct source inspection + exact passing tests 为准）
- **Standalone closeout**: `UX-01` + `UX-05` ✅（2026-03-14 standalone closeout，2026-03-15 template-sync follow-up，2026-03-16 bounded E5 eval-fixture repair，2026-03-17 provider-boundary follow-up；执行模型 `gpt-5-codex`；`hepar init` 与 `research-team scaffold` 已统一到 shared `ensure_project_scaffold` authority，canonical minimal root surface 锁定为 `project_charter.md` / `project_index.md` / `research_plan.md` / `research_notebook.md` / `research_contract.md` / `.mcp.json.example`，bounded naming audit 已收口为 `knowledge_base/` optional、`prompts/`/`team/`/`research_team_config.json` host-local、`references/` optional、`.hep/` provider-local。2026-03-15 follow-up 已补齐缺失 template authority：新增 `project_charter.md` / `research_plan.md` 与 `AGENTS.md` / `docs/*` 模板，移除 `project_scaffold.py` 内联 scaffold 文案 authority，最小 `project_index.md` 不再把 host/provider-local extras 作为默认链接面，并新增双向 anti-drift gate `test_scaffold_template_sync.py`。2026-03-16 bounded follow-up 进一步修复了未随主线迁移完成的 `E5-revision-compile-provenance` eval fixture surface：`case.json` 改为要求 `evals/fixtures/revision_project/research_contract.md`，最小 checked-in revision fixture `research_contract.md` 与 `paper/main.tex` 已补齐，新增 `test_revision_fixture_surface.py` 锁定旧名不得回流，并通过精确 `.gitignore` 例外只暴露该 fixture 的 `paper/main.tex`。2026-03-17 follow-up 进一步锁定 generic/provider 边界：`.mcp.json.example` 改为 provider-neutral `example-provider` / `PROVIDER_DATA_DIR` 占位，default full scaffold 不再预建 `.hep/`，仅 `--with-hep-provider` 才会创建 `.hep/` 并打开 `hep_workspace_gate`；同时 `check_project_map.py` / `update_project_map.py` 与 `research_contract.py` 的链接语义已对齐 shared minimal scaffold authority。最终 acceptance 现为 `packages/hep-autoresearch/tests` = 204 passed、`skills/research-team/tests` = 198 passed/1 skipped、prompt-required smokes 全过、`git diff --check` 通过、bounded negative grep 无旧名残留。formal review 最终以 `Opus + Gemini-3.1-Pro-Preview + OpenCode(kimi-for-coding/k2p5)` 收敛为 0 blocking：首轮 Gemini agentic review 因本地 tool-path 噪声/误用无效，已记录；随后使用同一 Gemini 模型的 packetized rerun 得到有效 `CONVERGED` 结果。2026-03-15 template-sync follow-up formal review R1 收敛为 0 blocking + 1 low-risk amendment，并已吸收为反向 orphan-template anti-drift gate；formal review R2 由同一 trio 收敛为 0 blocking / 0 amendments；2026-03-16 E5 bounded repair formal review 同样由该 trio 收敛为 0 blocking / 0 amendments；2026-03-17 provider-boundary follow-up 也由同一 trio 收敛为 0 blocking / 0 amendments。GitNexus pre/post `npx gitnexus analyze --force` 均成功；post-change `detect_changes` 可用但对 untracked fixture 文件仍有低覆盖，且 `impact` / `context` follow-up 仍会报 `Transport closed`，因此 exact verification 继续退回到 direct source inspection + exact acceptance。self-review 0 blocking）
- **Standalone closeout**: `NEW-05a-shared-boundary` ✅（`packages/shared/` 不再持有具体 `HEP_*` tool-name authority / HEP risk map / `hep://runs` helper；`packages/hep-mcp/` 本地 authority + wrappers 已落地。2026-03-18 bounded follow-up 进一步收拢了 `hep-mcp` 内部 provider-local `hep://runs` construction/parsing authority 到 `packages/hep-mcp/src/core/runArtifactUri.ts`，并新增 anti-drift gate，覆盖 template / concat（含 backticks）/ local helper / local parse helper / inline parse regex 回流。该 follow-up 的 formal review 使用要求的 trio `Opus + Gemini-3.1-Pro-Preview + OpenCode(kimi-for-coding/k2p5)`：`Gemini` 与 `K2.5` 直接在 R2 收敛为 0 blocking，tool-enabled `Opus` runner 两次 hang 未产出 JSON，随后同一 `Opus` 模型通过 packetized-evidence rerun 收敛为 0 blocking；self-review 0 blocking。GitNexus 仍会漏报新文件/新 helper callsites，因此 post-change exact verification 继续以源码 grep + scoped passing tests 为准）
...
- **Standalone closeout**: `NEW-05a-idea-core-domain-boundary` ✅（2026-03-16 formal rereview on `main` reopened the earlier 2026-03-10 closeout as `NOT_CONVERGED`; the current bounded repair now re-closes it on live code rather than narrative. `packages/idea-core/src/idea_core/engine/default_domain_pack.py` adds provider-neutral `generic.default.v1`, and `build_builtin_domain_pack_index()` now returns that neutral pack instead of an HEP-only built-in index. `DomainPackAssets.librarian_recipes` is now explicit required data; generic `build_default_librarian_recipe_book()` emits only `GENERIC_LITERATURE` / `GENERIC_EVIDENCE`, while HEP templates moved to provider-local `hep_librarian_recipe_book.py` and are injected only through explicit `build_builtin_hep_domain_pack_index()` use in `default_service()` and `rpc/server.py:main()`. Boundary tests now prove actual runtime behavior: `test_domain_pack_m30.py` and `test_retrieval_recipes_m33.py` inspect real non-HEP packets and assert no `INSPIRE` / `PDG` / HEP recipe IDs; `test_search_step_operator_families_m32.py`, `test_hep_constraints_m35.py`, and `test_compute_plan_rubric_m36.py` explicitly inject the HEP index before asserting HEP-only behavior; `test_formalism_registry_m31.py` now guards that the default built-in pack is `generic.default.v1`; and `test_novelty_delta_m34.py` locks the removal of `observable-1` from production novelty wording. Acceptance on the repaired worktree is fully green: prompt-required targeted tests, `test_formalism_registry_m31.py`, `make -C packages/idea-core validate`, `make -C packages/idea-core test` (`119 passed`), exact negative greps for `INSPIRE|PDG|inspire.generic.hep.v1|pdg.generic.hep.v1` in generic/default service files and `observable-1` in production `coordinator.py`, and `git diff --check`. Formal review converged with `Opus + Gemini-3.1-Pro-Preview + OpenCode(kimi-for-coding/k2p5)` at 0 blocking: aggregate swarm produced usable `Opus` + `K2.5`, while aggregate Gemini hung without output and was explicitly recovered by rerunning the same reviewer with a flattened packet and proxy-first `generateContent`; the sole low-risk amendment (leftover `observable-1` text in an M31 test fixture) was absorbed and revalidated. Self-review also passed with 0 blocking. GitNexus freshness pre/post was satisfied via `npx gitnexus analyze` and `npx gitnexus analyze --force`, but post-change MCP graph calls still failed with `Transport closed`, so final exact verification remained direct source inspection + passing tests. No `meta/REDESIGN_PLAN.md` or `.serena/memories/architecture-decisions.md` update was required because this repair restored the already-declared provider-neutral boundary rather than introducing a new stable invariant）
- **Standalone closeout**: `NEW-05a-stage3` ✅（2026-03-16 authority-seam bounded repair 已完成：enhanced rereview `NOT_CONVERGED` 指出的 generic `idea-engine` authority leakage 已在严格受限的 `search.step` 面内修复。generic `search-operator.ts` / `librarian-recipes.ts` / `domain-pack.ts` 现仅保留 provider-neutral types/helpers；当前 HEP operator/runtime/catalog/recipe authority 已下沉到 provider-local `domain-pack-registry.ts`、`hep-domain-pack.ts`、`hep-search-runtime.ts` 与 `hep-librarian-recipe-book.ts`，`search-step-service.ts` 通过 registry seam 加载 runtime + recipe book，不再内联 HEP authority。`packages/idea-core/src/idea_core/engine/schemas/librarian_evidence_packet_v1.schema.json` 已将 `provider` / `api_source` 打开为非空字符串，`packages/idea-engine/tests/authority-seam.test.ts` 与 Python `test_retrieval_recipes_m33.py` 共同锁定 generic seam 可接受 non-HEP provider，而 `search-step-parity.test.ts` 明确将 golden fixture 叙事降格为 “through built-in domain-pack seam” 的 provider-local parity surface。acceptance 全绿：`git diff --check`、`pnpm install --frozen-lockfile`、`pnpm --filter @autoresearch/idea-engine build`、targeted parity suite、full `idea-engine` suite (`33 passed`)、Python targeted tests 与 `make -C packages/idea-core validate` 均通过；exact negative grep 也确认三份 generic service 文件已无 `hep.` / `INSPIRE` / `PDG` / `hep_operator_families_m32` / `hep_builtin_domain_packs` 残留。formal review 经 `Opus + Gemini-3.1-Pro-Preview + OpenCode(kimi-for-coding/k2p5)` 在 `.review/2026-03-16-new05a-stage3-authority-seam-bounded-repair/swarm-r1/` 收敛为 `0` blocking / `0` amendments；wrapper-format contract warnings 仅为 informational。self-review 亦为 `0` blocking。GitNexus pre/post `npx gitnexus analyze --force` 均成功，但 post-change `detect_changes` / `context` 仍报 `Transport closed`，所以 final exact verification 继续以 direct source inspection + passing acceptance 为准。implementation prompt 已保存为 `meta/docs/prompts/prompt-2026-03-16-new05a-stage3-authority-seam-bounded-repair.md`；implementation commit 已在用户授权后记录为 `861b64e` (`Repair NEW-05a stage3 authority seam`)；本批未扩到 `eval.run` / `rank.compute` / `node.promote` / broader Stage 3 migration / `NEW-07` / `EVO-13`，且无新增稳定架构不变量，因此未更新 `meta/REDESIGN_PLAN.md` 或 `.serena/memories/architecture-decisions.md`）
...
- **Standalone enhanced rereview**: `NEW-SEM-10` + `NEW-SEM-13` ✅（2026-03-18，在干净 `main` worktree 上按 live code / callers / tests / eval fixtures 重新判定，先明确 reopen 旧 closeout：provider-local grouping labels 先前仍会经 `analyzePapers.ts::extractTopics()` 与 `grouping.ts::groupByMethodology()` 进入 public topic/method wording，challenge taxonomy wording 先前仍会经 `challengeExtraction.ts::renderMethodologyChallenges()` 进入 public narrative；随后 bounded repair 已把 SEM-10 public surface 收口到 evidence-first keywords + generic `Method cluster N` naming，把 SEM-13 public surface 收口到 source-phrase summaries / quoted narrative wording，并将 eval authority 分别改锁为 cluster membership + `public_keyword_leak_rate = 0` 与 `challenge_markers` + `taxonomy_wording_leak_rate = 0`。本轮 formal review 最终以要求的 trio `Opus + Gemini-3.1-Pro-Preview + OpenCode(kimi-for-coding/k2p5)` 收敛为 0 blocking：`Opus` = `CONVERGED_WITH_AMENDMENTS`，`Gemini` = `CONVERGED`，exact `kimi-for-coding/k2p5` reviewer = `CONVERGED`；本轮吸收的 low-risk amendments 仅限删除 dead-code `humanizeSemanticLabel` / `HUMANIZED_LABELS` 与 `HUMANIZED_CHALLENGES`。self-review 0 blocking；GitNexus CLI freshness pre/post `npx gitnexus analyze --force` 均满足，但 MCP `list_repos` / `context` / `impact` follow-up 仍 `Transport closed`，因此 final exact verification 继续以 direct source inspection + exact scoped tests/build 为准；未扩到 `NEW-SEM-08`、`NEW-RT-06` 或 broader retrieval/runtime lane）
...
- **Standalone closeout**: `NEW-RT-04` ✅（2026-03-17，执行模型 `gpt-5-codex`；在 2026-03-15 bounded repair 纠偏基础上，最终收口不是停在 `executeDelegatedAgentRuntime()` library export，而是落到真实 shared tool surface `orch_run_execute_agent`：`packages/orchestrator/src/orch-tools/index.ts` 将其注册进 `ORCH_TOOL_SPECS`，`packages/orchestrator/src/orch-tools/agent-runtime.ts` 通过 `executeDelegatedAgentRuntime()` 真正消费 `AgentRunner` + `RunManifestManager`，`packages/hep-mcp/src/tools/registry/projectExtensions.ts` 与 `packages/hep-mcp/src/tools/dispatcher.ts` 则提供 host `createMessage` + loopback `callTool`，使 `resume_from` / `last_completed_step` 通过 production `handleToolCall(...)` 路径改变实际执行行为。新增 host-path contract `packages/hep-mcp/tests/contracts/orchRunExecuteAgent.test.ts` 锁定 manifest persist + crash/re-entry skip path；后续又吸收了 cold-baseline anti-drift follow-up：generic/shared authority 仍停留在 `@autoresearch/shared` / `@autoresearch/orchestrator` package entrypoints，HEP 只保留 host adapter/consumer，不回灌 generic tool-name authority；新增 `packages/hep-mcp/tests/contracts/sharedOrchestratorPackageExports.test.ts` 直接验证 package export seam，root `npm run accept:new-rt-04` 则从 cold workspace baseline 清空 ignored `dist/` + `tsbuildinfo` 后按依赖顺序重建 `shared -> orchestrator -> sibling providers -> hep-mcp` 再执行 exact host-path acceptance，防止 warmed worktree 掩盖 export drift。formal review-swarm R1 暴露 dead-helper blocker；R2 由 `Opus + Gemini-3.1-Pro-Preview + OpenCode(kimi-for-coding/k2p5)` 收敛为 0 blocking（`CONVERGED_WITH_AMENDMENTS` / `CONVERGED` / `CONVERGED`），self-review 0 blocking。GitNexus pre/post `npx gitnexus analyze --force` 成功，但 MCP `context` / `detect_changes` 仍 `Transport closed`，因此 exact verification 继续以 direct source inspection + passing acceptance 为准；本次 cold-baseline follow-up 中 GitNexus CLI reindex 再次成功，但 MCP follow-up 仍失败，故仍按该 fallback 记录。本批未扩到 `NEW-05a`、`NEW-07`、`EVO-13` 或 provider-local runtime wrapper；已新增稳定 closeout invariant，因此 `.serena/memories/architecture-decisions.md` 现已同步）
- **Standalone bounded repair**: `hep-mcp` tooling import closure ✅（2026-03-16：按 `meta/docs/prompts/prompt-2026-03-16-mcp-tooling-import-repair.md` 重新审计当前 worktree 后确认，残留 failure 不是新的 generic-entrypoint / broker lane，也不是 `hep-mcp` provider import wiring 的 live code bug。当前根因是新 worktree 缺少 workspace install 与 prerequisite build artifacts：起始状态下 `node_modules` 缺失，`packages/{shared,orchestrator,arxiv-mcp,openalex-mcp,pdg-mcp,zotero-mcp,hepdata-mcp}/dist` 均为空；先后执行 `pnpm install --frozen-lockfile`、`pnpm --filter @autoresearch/shared build`、`pnpm --filter @autoresearch/orchestrator build`、五个 provider prerequisite builds 后，prompt 目标 gate `pnpm --filter @autoresearch/hep-mcp test -- tests/toolContracts.test.ts` 与 `pnpm --filter @autoresearch/hep-mcp build` 已通过，`git diff --check` 也通过。期间第一处真实报错先是 provider build 缺 `@autoresearch/shared` entry，然后是 `hep-mcp` test/build 缺 `@autoresearch/orchestrator` entry；在 prerequisite build closure 完成后，provider `tooling` / `shared/zotero` subpath imports 解析恢复正常。formal review 的唯一直接相关 low-risk amendment 已同批吸收：`packages/hep-mcp/tsconfig.json` 现补齐 `../pdg-mcp` 与 `../zotero-mcp` references，用以对齐现有 import surface；该 gap 属于 pre-existing tsconfig completeness 问题，不改变当前 `pnpm -r build` orchestration 下“failure 本质是 environment/baseline recovery”的结论。结论：本批不需要把问题升级成 generic-entrypoint 新 lane，也不需要改写现有 package contract / import surface；formal review 与 self-review 已按此 bounded 结论收尾，`.serena/memories/architecture-decisions.md` 与 `meta/REDESIGN_PLAN.md` 均无需更新）
...
- **Standalone follow-up**: `NEW-RT-04` destructive `_confirm` contract ✅（2026-03-17，执行模型 `gpt-5-codex`；在同日 live-entrypoint closeout 基础上补齐 shared destructive contract：`packages/orchestrator/src/orch-tools/schemas.ts` 为 `OrchRunExecuteAgentSchema` 增加 `_confirm: z.literal(true)`，`packages/orchestrator/src/orch-tools/index.ts` 明确 tool 描述为 destructive 且 requires `_confirm: true`，`packages/hep-mcp/tests/contracts/orchRunExecuteAgent.test.ts` 新增 production dispatcher path 下缺失 `_confirm` → `CONFIRMATION_REQUIRED` 的 host-path contract，`packages/hep-mcp/tests/toolContracts.test.ts` 明确锁定 `ORCH_RUN_EXECUTE_AGENT: 'destructive'`。由于保留了 `full` exposure，当前 docs/generated counts 已同步到 live built-in registry：`README.md`、`docs/README_zh.md`、`docs/TOOL_CATEGORIES.md`、`docs/PROJECT_STATUS.md` 与 `packages/hep-mcp/tool_catalog.{standard,full}.json` 现为 `standard=67` / `full=86`。follow-up acceptance 通过：原 RT04 scoped build/test gates 继续通过，新增 `pnpm --filter @autoresearch/hep-mcp test -- tests/toolContracts.test.ts tests/docs/docToolDrift.test.ts` 与 `pnpm --filter @autoresearch/hep-mcp docs:tool-counts:check` 也已通过。formal review-swarm 经 `Opus + Gemini-3.1-Pro-Preview + OpenCode(kimi-for-coding/k2p5)` 收敛为 0 blocking，唯一 low-risk amendment 是把 tracker / AGENTS 从 in-progress 收口到 done 并补记 follow-up 结果，已同轮吸收；self-review 0 blocking。GitNexus pre/post `npx gitnexus analyze --force` 仍成功，但 post-change `context` / `impact` / `detect_changes` 持续 `Transport closed`，因此 exact verification 继续以 direct source inspection + passing acceptance 为准。本批未改 `NEW-RT-07` 逻辑，也未更新 `meta/REDESIGN_PLAN.md` 或 `.serena/memories/architecture-decisions.md`，因为 closeout judgment 与长期架构边界均未改变）
...
This project is indexed by GitNexus as **autoresearch-lab-new-sem10-13-rereview** (8485 symbols, 22868 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/autoresearch-lab-new-sem10-13-rereview/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/autoresearch-lab-new-sem10-13-rereview/context` | Codebase overview, check index freshness |
| `gitnexus://repo/autoresearch-lab-new-sem10-13-rereview/clusters` | All functional areas |
| `gitnexus://repo/autoresearch-lab-new-sem10-13-rereview/processes` | All execution flows |
| `gitnexus://repo/autoresearch-lab-new-sem10-13-rereview/process/{name}` | Step-by-step execution trace |
