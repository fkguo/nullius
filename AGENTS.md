...
- **Standalone pending**: `NEW-05a-hep-semantic-authority-deep-cleanup`（深度审计已落地到 `meta/docs/2026-03-10-hep-semantic-authority-deep-audit.md`，program prompt 已落地到 `meta/docs/prompts/prompt-2026-03-10-hep-semantic-deep-cleanup.md`；当前 source 仍存在 active authority：`idea-core` 的 `hep.bootstrap` / `bootstrap_default` / `HEP_COMPUTE_RUBRIC_RULES` / `toy_laptop`，以及 `hep-mcp` 的 review/paper/question/assumption/conflict/grouping/challenge closed lexicon authority。新顺序为 semantic deep cleanup A-E -> residual batch2 closeout -> batch3；不得再按“formalism 清理后直接 batch2/batch3”执行）
- **Standalone pending**: `NEW-05a-idea-core-domain-boundary`（按 `meta/docs/prompts/prompt-2026-03-09-batch2-idea-core-domain-boundary.md` 的局部代码清理仍在当前工作树且 `make -C packages/idea-core test` 全绿，但 formal closeout 再次暂停：`formalism` blocker 已清除，随后深审又发现 `hep.bootstrap` / `bootstrap_default` / `toy_laptop` / HEP compute rubric 仍在 generic/core path 生效。下一步不是直接 closeout batch2，而是先执行 semantic deep cleanup 的 Batch A，必要时将其与 batch2 residual scope 合并后再正式收口）
...
This project is indexed by GitNexus as **autoresearch-lab** (9844 symbols, 22937 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
