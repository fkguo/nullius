...
- **Bounded repair**: `UX-02` 维持 closeout，`NEW-COMP-01` 已恢复 closeout，`NEW-CONN-04` 仅需文档同步后继续维持 closeout，`NEW-RT-04` 已按真实状态降级为 `in_progress`（2026-03-15 retro-closeout repair：不扩成新 runtime lane，只修两处 blocker + 一处文档漂移。`NEW-COMP-01` 现已把 `meta/docs/computation-mcp-design.md` 与 live authority 对齐为 `write` risk / no `_confirm`，并新增 `packages/hep-mcp/tests/contracts/ingestSkillArtifactsContract.test.ts` 锁定 contract；`NEW-CONN-04` 的 `meta/REDESIGN_PLAN.md` `next_actions` 叙事已同步到 live `hep_run_plan_computation -> inspire_search -> hep_project_build_evidence`；`NEW-RT-04` 经 direct source inspection、grep 与 GitNexus `context(AgentRunner|RunManifestManager)` 再确认仍无 shared entrypoint / production caller，因此不得继续维持 closeout，只保留 library-level foundation 叙事。Scoped gates passed：`git diff --check`、`pnpm --filter @autoresearch/orchestrator test -- tests/run-manifest.test.ts tests/agent-runner-manifest.test.ts`、`pnpm --filter @autoresearch/orchestrator build`、`pnpm --filter @autoresearch/hep-mcp test -- tests/core/ingestSkillArtifacts.test.ts tests/contracts/ingestSkillArtifactsContract.test.ts`、`pnpm --filter @autoresearch/hep-mcp test -- tests/core/createFromIdea.test.ts tests/contracts/ideaRunsIntegrationContract.test.ts`。Canonical baseline failures remain outside scope: `pnpm --filter @autoresearch/hep-mcp test -- tests/toolContracts.test.ts` 与 `pnpm --filter @autoresearch/hep-mcp build` 仍因缺失 sibling package tooling/types 失败。GitNexus pre/post `npx gitnexus analyze --force` 均成功，`detect_changes` = LOW。formal review 最终由 `Opus + Gemini-3.1-Pro-Preview + OpenCode(kimi-for-coding/k2p5)` 收敛为 0 blocking；Gemini 首次 agentic rerun 超时、首次 direct rerun 仅返回 hook noise，随后第二次 direct rerun 成功返回有效 JSON。self-review 0 blocking，未更新 `.serena/memories/architecture-decisions.md`）
...
This project is indexed by GitNexus as **autoresearch-lab-new05a-idea-core-domain-boundary-repair** (8448 symbols, 22715 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/autoresearch-lab-new05a-idea-core-domain-boundary-repair/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/autoresearch-lab-new05a-idea-core-domain-boundary-repair/context` | Codebase overview, check index freshness |
| `gitnexus://repo/autoresearch-lab-new05a-idea-core-domain-boundary-repair/clusters` | All functional areas |
| `gitnexus://repo/autoresearch-lab-new05a-idea-core-domain-boundary-repair/processes` | All execution flows |
| `gitnexus://repo/autoresearch-lab-new05a-idea-core-domain-boundary-repair/process/{name}` | Step-by-step execution trace |
