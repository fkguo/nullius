...
- **Current active batch (2026-03-11)**: `Batch 15 = NEW-SEM-08` 已完成 bounded closeout；archive-first SOTA preflight、Python-side semantic packet curation contract（candidate expansion -> semantic adjudication -> fail-closed render plan）、formal review（Gemini 不可用；人类批准 fallback；实际使用 `Opus + OpenCode(kimi-for-coding/k2p5)`）与 self-review 均已完成，默认下一批仍是 `Batch 16 = NEW-SEM-11 + NEW-SEM-12`。本轮未重开 `NEW-LOOP-01`、runtime/root de-HEP 或既有 semantic cleanup lane；`research-team` sidecar smoke hang 与 full-contract `full_access` stub mismatch 仍是已知 pre-existing blocker。
