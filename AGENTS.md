...
- canonical generic lifecycle + workflow-plan entrypoint 现为 `autoresearch`（`@autoresearch/orchestrator` bin），当前覆盖 `init/status/approve/pause/resume/export/workflow-plan`；其中 `workflow-plan` 是面向已初始化外部 project root 的 stateful literature-plan front door。
...
This project is indexed by GitNexus as **autoresearch-lab** (12924 symbols, 28822 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
