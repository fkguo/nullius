...
| `@autoresearch/hep-mcp` | 当前最成熟的首个 provider family：INSPIRE-HEP + evidence-first HEP workflow（`hep_*`、`zotero_*`、`pdg_*`） (67 std / 93) | Active |
...
工具数量：**`standard` 模式 67 个**（默认：收敛后的紧凑工具面）与 **`full` 模式 93 个**（额外暴露 advanced 工具）。
