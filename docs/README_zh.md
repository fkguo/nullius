...
- project-local durable memory，例如 `research_plan.md`、`research_contract.md`、已经有实质内容的 `research_notebook.md`，以及 `reports/` 下已登记的不可变报告
...
```
  ...
  当前仓库是本地 workspace，不是已发布的全局 npm CLI。`pnpm install:cli` 在 Windows 写入受管理的 `%APPDATA%\npm\nullius.cmd`，在 POSIX 写入 `~/.local/bin/nullius`；除非显式传入 `--force`，它不会覆盖未受管理的命令，并会报告目标目录是否在 `PATH` 中。`nullius init` 也会写入 project-local launcher；无全局 wrapper 时，POSIX 使用 `./.nullius/bin/nullius status --json`，原生 Windows 使用 `.\.nullius\bin\nullius.cmd status --json`。
...
- delegated agent 执行现在在 tool 边界持久化：attempt 明确区分 `not_started`、`outcome_unknown` 与 `committed`；副作用结果未知时停机交 operator 核对，已提交的 MCP result 原样 replay 而不再次派发。direct 与 team journal 分别位于 `artifacts/delegated-runs/direct/<run_id>/` 和 `artifacts/delegated-runs/team/<runtime_run_id>/`，caller 自选的 root/direct run id 无法冒充 team checkpoint；approval resolver 只允许 host/operator 调用。`nullius run` 启动配置的 stdio MCP server 时，非敏感配置写入 `NULLIUS_RUN_MCP_ENV_JSON`，凭据写入 `NULLIUS_RUN_MCP_CREDENTIALS_JSON`，必需凭据名可用 `NULLIUS_RUN_MCP_REQUIRED_CREDENTIALS_JSON` 声明；缺值会在 spawn 前失败。原生 Windows 已支持 lifecycle、project-local recovery 与 native computation，但配置的 stdio MCP workflow dispatch 仍要求 process-tree containment，因此会在 spawn 前 fail closed；`best_effort` 只属于显式的底层 `McpClient` 选项，不是公开 workflow 默认值。
