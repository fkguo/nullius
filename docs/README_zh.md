...
| `@autoresearch/hep-mcp` | 当前最成熟的首个 provider family：INSPIRE-HEP + evidence-first HEP workflow（`hep_*`、`zotero_*`、`pdg_*`） (66 std / 83) | Active |
...
工具数量：**`standard` 模式 66 个**（默认：收敛后的紧凑工具面）与 **`full` 模式 83 个**（额外暴露 advanced 工具）。

### 工具暴露模式

| 模式 | 工具数 | 说明 |
|------|--------|------|
| `standard` | 66 | 默认：紧凑、推荐 |
| `full` | 83 | `standard` + advanced 工具 |
