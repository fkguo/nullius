...
- **Experimental TS idea runtime surface**: the idea-engine search/eval runtime is archived; `@nullius/idea-mcp` remains campaign-lifecycle oriented on explicit external data roots, while `@nullius/idea-engine` keeps node posterior/lifecycle updates and posterior-based ranking and promotion inside the runtime contract, not as a root front door or a new default capability lane; contracts + store are retained, and scoring consumes an external belief-graph posterior (pinned tool, current pin gaia-lang==0.5.0a4)
...
- **Experimental idea campaign workflow**: `idea_campaign_init` with `idea_campaign_topup` / `idea_campaign_pause` / `idea_campaign_resume` / `idea_campaign_complete` on `idea-mcp`; node posterior/lifecycle updates, posterior ranking, and promotion stay engine-side (`node.set_posterior`, `node.set_lifecycle`, `rank.compute`, `node.promote`); the archived search/eval phase does not reopen island-search scoring, memory graph, positive memory retrieval, broad retrieval, generic eval platform, or front-door expansion
