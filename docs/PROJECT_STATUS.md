...
- **Resume context seam**: `orch_run_status` / `orch_run_export` now surface a machine-readable `resume_context` plus `current_run_workflow_outputs`, so reconnecting agents can recover the current run and reuse bounded workflow outputs without manually rerunning atomic tools
