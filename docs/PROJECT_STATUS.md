...
- **Execution-mode + conversational-decision seam**: `nullius init --mode=<engine|file>` declares where project truth lives (engine run/approve lifecycle vs file surfaces with an idle `run_status`), surfaced as `execution_mode` in the status receipt; `nullius decision <record|pending|list>` appends human decisions made in conversation to `.nullius/decisions.jsonl`, open questions stay in the receipt until resolved, an undeclared project whose engine state stays frozen while dated run evidence accumulates gets a diagnostic-only drift hint, and declared file-mode drops the `RECOVERY_PLAN_FOCUS_UNAVAILABLE` reconnect noise — no new blocking gate anywhere
...
- **Conversational decision workflow**: `nullius decision record "<what was decided>"` / `nullius decision pending "<question>"` / `nullius decision list` append to and read the project's `.nullius/decisions.jsonl` ledger — the engine-visible record of approvals and choices made in conversation, replacing hand-built markdown decision ledgers without adding any gate
