...
- **Pre-approval integrity-receipt seam**: `nullius integrity-record` records an M1–M7 AI-failure-mode receipt bound to a pending `approval_id`; the `approve` gate is fail-closed on it (`INTEGRITY_RECEIPT_REQUIRED`, skippable only via `NULLIUS_INTEGRITY_VERIFY`), so an A5 final-conclusions approval cannot be resolved without a recorded receipt
