...
- **Conversational decision workflow**: `nullius decision record "<what was decided>"` / `nullius decision pending "<question>"` append to the project's `.nullius/decisions.jsonl` ledger and `nullius decision list` reads it back — the engine-visible record of approvals and choices made in conversation, replacing hand-built markdown decision ledgers without adding any gate
