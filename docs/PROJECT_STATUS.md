...
- **Experimental TS idea runtime surface**: the current idea-engine phase is closed; `@nullius/idea-mcp` remains campaign/search/eval oriented on explicit external data roots, while `@nullius/idea-engine` keeps post-search `rank.compute` / `node.promote` and bounded negative failure-library reflection inside the runtime contract, not as a root front door or a new default capability lane
