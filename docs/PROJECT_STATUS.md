...
- **Generic follow-up continuation MCP front door**: `orch_run_progress_followups` advances exactly one computation-generated follow-up task through the generic delegated runtime surface; delegated `idea` feedback is now live, while `literature_followup` remains explicit deferred-skip
...
- **Generic follow-up continuation workflow**: `orch_run_progress_followups` is the canonical bounded consumer for computation-generated feedback and writing/review follow-ups; it progresses one follow-up task per call and does not invent scheduler or literature-query fallback semantics
