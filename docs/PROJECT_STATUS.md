...
- **Agent-client continuation workflow**: `research-harness` recovers project truth first, then routes to `autoresearch`, `research-team`, or `hep-mcp` as appropriate; `research-team` outputs are incomplete until durable conclusions are summarized or linked back into project artifacts and status files
