...
- `nullius init --mode=<engine|file>` 声明项目真相所在（engine=run/approve lifecycle 驱动；file=手工/外部 runner 执行、`run_status` 长期 `idle` 属正常），可随时重跑改声明。`nullius decision record|pending` 追加写 `.nullius/decisions.jsonl`，`decision list` 只读回放，`decision land` 只在分支尾已进入权威主干后原子落号。手动验收至少覆盖：
  - 在开发仓外临时 root 运行 `nullius init --runtime-only --mode=file`，再执行 `nullius decision pending "<问题>"`；打印的 id 必须是 6 字符 Crockford-base32 handle，`status --json` 必须把它计入 `pending_count`、`open_count` 和 `unlanded_count`。随后用该 handle 执行 `nullius decision record "<决定>" --resolves <handle>`，断言 open 数归零，两个 provisional 条目仍完整进入 decided/pending 计数。
  - 执行 `nullius decision land --json`，断言它按当前主干 ledger 顺序分配后续 `D<n>`、把 answer 的 `resolves` 改成 landed pending id、在各条目保留 `provisional_id` 映射，并把 `unlanded_count` 清零；JSON 中的 `rewritten_resolution_count` 必须与实际改写数一致。若所有 entry 已有 durable id、但迟到的 branch resolution 仍指向旧 handle，land 仍须通过保留映射只改写该引用；再次对规范 ledger 执行才是零改写的幂等 no-op。展示顺序按毫秒精度 `ts`，不按随机 handle。
  ...
  - 复制任意条目制造同 id 两行；`decision list` 必须非零退出并列出 id 与物理行号，`status --json` 的 `decision_ledger.duplicate_id_count` 必须为 1，`--resolves <重复 id>` 与 `decision land` 都必须拒绝，且 land 失败前后 ledger 字节完全相同。
  - 构造两条不同 `D<n>` 但保留同一 `provisional_id` 的记录，并另测一个当前 handle 与既有 `provisional_id` 重名；两者都必须由 `decision_ledger.ambiguous_provisional_id_count` 与具名行号显式报告，`decision list` 非零退出，相关 resolution 被隔离，`decision land` 零改写拒绝。正常铸号必须避让本地已有 `provisional_id`。
  - `status` 文本必须渲染 `execution_mode`、open 项与 `decisions_unlanded`；file 模式下不得出现 `RECOVERY_PLAN_FOCUS_UNAVAILABLE`。未声明 mode、引擎状态冻结且已有日期 run 目录时，`project_surface_drift` 应出现 `EXECUTION_MODE_UNDECLARED_LOOKS_FILE_MODE`（诊断提示，不阻塞任何命令）。
