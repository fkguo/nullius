...
| `file://` | `@nullius/idea-engine` | `packages/idea-engine/src/service/{node-promote-executor,rank-compute-executor}.ts` emit via `pathToFileURL(...).href`; `handoff`/`ranking` refs are resolved by `packages/orchestrator/src/computation/idea-engine-feedback.ts` and `packages/hep-mcp/src/tools/idea-staging.ts` | Tool-return/cross-package artifact-ref handoff identifiers, not MCP resources | `file://<absolute-artifact-path>` for idea-engine `handoff` / `ranking` artifact refs | Standard local file URL of an idea-engine artifact path. `handoff`/`ranking` refs are consumed by the orchestrator/HEP idea-staging bridge. It is not an owned project/run namespace and not an MCP resource. |
| `project://` | `@nullius/idea-engine`, `skills/idea-posterior`, `skills/research-team` | Generation pack refs emitted and resolved by `packages/idea-engine/src/{store/engine-store.ts,service/import-generated-executor.ts,service/import-generated-recovery.ts}`; Gaia package refs emitted by `skills/idea-posterior/scripts/run_infer_and_extract.py`; Gaia and bound literature-ledger refs validated by `skills/idea-posterior/scripts/posterior_writeback.py`; raw bibliography and canonical-metadata provenance refs validated by `skills/research-team/scripts/gates/check_literature_trace.py` | Machine-portable project-relative content-pinned references stored in idea-store nodes/posteriors or literature coverage receipts, not MCP resources | `project://<percent-encoded project-relative path>#sha256:<pack_hash>` for generated packs; `project://<percent-encoded project-relative path>#sha256:<exact_ir_json_hash>` for `gaia_package_ref`; `project://<project-relative path>#sha256:<exact_artifact_hash>` for combined literature ledgers, raw bibliography manifests, and archived provider metadata/triangulation blocks | Resolves against the enclosing project root (explicit project root when provided; otherwise the owning tool's project-root inference). The path must stay inside the project root (and, for idea-engine pack artifacts, inside the engine store root), with no empty or `..` segments. The fragment pins the referenced content's exact bytes. Literature survey bibliography and method summaries must reference the same combined ledger; writeback resolves it and recomputes provider accounting, candidate/core closure, bibliography, method, and graph status before storage. Deliberately relative: research projects sync across machines, so machine-absolute forms go stale. |
