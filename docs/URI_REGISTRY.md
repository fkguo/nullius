...
| `project://` | `@nullius/idea-engine`, `skills/idea-posterior` | Generation pack refs emitted and resolved by `packages/idea-engine/src/{store/engine-store.ts,service/import-generated-executor.ts,service/import-generated-recovery.ts}`; Gaia package refs emitted by `skills/idea-posterior/scripts/run_infer_and_extract.py`, validated and resolved by `skills/idea-posterior/scripts/posterior_writeback.py` | Machine-portable project-relative content-pinned references stored in idea-store nodes/posteriors, not MCP resources | `project://<percent-encoded project-relative path>#sha256:<pack_hash>` as `node.import_generated` `pack_artifact_ref` / generated nodes' `operator_trace.inputs.pack_artifact`; `project://<percent-encoded project-relative path>#sha256:<exact_ir_json_hash>` as the `gaia_package_ref` of `node.set_posterior` payloads | Resolves against the enclosing project root (explicit project root when provided; otherwise the owning tool's project-root inference). The path must stay inside the project root (and, for idea-engine pack artifacts, inside the engine store root), with no empty or `..` segments. The fragment pins the referenced content: the generation pack hash for `pack_artifact_ref`, or SHA-256 of the exact compiled `.gaia/ir.json` bytes for `gaia_package_ref`. The latter includes exported-root metadata that Gaia 0.5.0a4's internal `ir_hash` omits. Deliberately relative: research projects sync across machines, so machine-absolute forms go stale. |
