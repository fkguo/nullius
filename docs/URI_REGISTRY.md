...
| `pdg://` | `@autoresearch/pdg-mcp` | `packages/pdg-mcp/src/artifacts.ts`; `packages/pdg-mcp/src/tools/registry.ts` | Tool-return/local artifact identifiers, not MCP resources | `pdg://artifacts/{artifact_name}` | PDG local artifact cache identifiers only. Full artifacts live under `PDG_DATA_DIR/artifacts`; the server does not expose MCP resources. |
| `orch://` | `@autoresearch/orchestrator` | `packages/orchestrator/src/orch-tools/{approval,control,create-status-list,run-read-model}.ts`; `packages/orchestrator/src/workflow-runtime.ts`; `packages/orchestrator/src/cli-run.ts` | Tool-return lifecycle/read-model/artifact identifiers | `orch://runs/{run_id}`; `orch://runs/{run_id}/approvals/{approval_dir}`; `orch://runs/{run_id}/artifact/{artifact_path}`; `orch://runs/export` | Orchestrator lifecycle/read-model/export summaries and orchestrator-owned workflow/result artifacts only. It does not own HEP research artifact payloads. |
...
| `file://` | `@autoresearch/idea-engine` | `packages/idea-engine/src/service/{node-promote-executor,eval-run-executor,rank-compute-executor,search-step-executor}.ts` emit via `pathToFileURL(...).href`; resolved by `packages/orchestrator/src/computation/idea-engine-feedback.ts` and `packages/hep-mcp/src/tools/idea-staging.ts` | Tool-return/cross-package artifact-ref handoff identifiers, not MCP resources | `file://<absolute-artifact-path>` for idea-engine `handoff` / `scorecards` / `ranking` / `evidence_packet` / `operator_trace` artifact refs | Standard local file URL of an idea-engine handoff artifact path, consumed by the orchestrator/HEP idea-staging bridge. It is not an owned project/run namespace and not an MCP resource. |
