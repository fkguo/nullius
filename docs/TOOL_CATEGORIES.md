# Tool Categories（standard=74 / full=86）
...
建议入口：
- Draft Path：`docs/WRITING_RECIPE_DRAFT_PATH.md`
- Client Path：`docs/WRITING_RECIPE_CLIENT_PATH.md`
...
| 端到端深度研究 + 写作 | `inspire_deep_research` (`mode=write`) | Evidence-first，需 `run_id` |
...
| 风格语料查询 | `inspire_style_corpus_query` | 需已构建本地 corpus index（`standard` 仅查询；构建在 `full` 且为 experimental） |
| PDG 粒子数据 | `pdg_find_particle`, `pdg_get` 等 | 离线；需 `PDG_DB_PATH` |
| Zotero 文献管理 | `zotero_local`, `zotero_find_items` 等 | 需 `HEP_ENABLE_ZOTERO=1` |

> Note（Phase 3）: 导航能力统一为 `inspire_research_navigator`（`mode=discover|field_survey|topic_analysis|network|experts|connections|trace_source|analyze`）；LaTeX 解析能力独立为 `inspire_parse_latex`（Evidence-first：要求 `run_id`，返回 artifact URI + 摘要）。
...
5. `inspire_deep_research(mode='write')` 端到端写作
6. `hep_export_project` 导出
...
- `hep_project_create`
- `hep_run_create`
- `hep_run_writing_submit_section_candidates_v1` + `hep_run_writing_submit_section_judge_decision_v1`（章节候选提交 + 评审选择；会触发 verifier）
- `hep_run_writing_submit_review`（当写作流水线要求审稿时）
- `hep_run_writing_integrate_sections_v1`（把已通过的 sections 集成到 `writing_integrated.tex`，并做 LaTeX compile gate）
- `hep_export_project`
- （可选：投稿闭环）`hep_export_paper_scaffold` → `hep_import_paper_bundle`
...
**Evidence index（run 级 LaTeX chunks + BM25 index）**
...
- `hep_run_build_writing_critical`（conflicts/stance/evidence_grades 资产化）
...
- `zotero_add`
- `zotero_confirm`
- `hep_import_from_zotero`（导入到 vNext run 的 mapping）

> 备注：已移除 full-only 的细粒度 `zotero_*` 工具；统一使用 `zotero_local` 的 `mode` 分派。

## G) vNext 写作（run-based；Evidence-first）

> vNext 写作流水线以 `hep_run_writing_*` 为主，强调：N-best 候选 + Judge 决策 + verifier + 产物落盘（artifacts）。

**Outline / Paperset**
- `hep_run_writing_create_paperset_curation_packet` → `hep_run_writing_submit_paperset_curation`
- `hep_run_writing_create_outline_candidates_packet_v1` → `hep_run_writing_submit_outline_candidates_v1`
- `hep_run_writing_create_outline_judge_packet_v1` → `hep_run_writing_submit_outline_judge_decision_v1`

**Evidence / Token**
- `hep_run_writing_build_evidence_packet_section_v2`（可选：`hep_run_writing_submit_rerank_result_v1` 提交 host LLM rerank 结果）
- `hep_run_writing_create_token_budget_plan_v1`
- `hep_run_writing_token_gate_v1`

**Section writing（Client Path 常用）**
- `hep_run_writing_create_section_write_packet_v1`（生成 prompt + evidence context）
- `hep_run_writing_create_section_candidates_packet_v1` → `hep_run_writing_submit_section_candidates_v1`
- `hep_run_writing_create_section_judge_packet_v1` → `hep_run_writing_submit_section_judge_decision_v1`

**Refinement / Integration**
- `hep_run_writing_refinement_orchestrator_v1`
- `hep_run_writing_create_revision_plan_packet_v1` → `hep_run_writing_submit_revision_plan_v1`
- `hep_run_writing_integrate_sections_v1`

## H) INSPIRE（网络工具：检索/分析/写作工作流）
...
- `inspire_style_corpus_query`（本地语料风格证据）

## Phase 4 Notes（非 4.2）

- 4.1：参考文献审计仅做 usability check，不作为研究质量阻断。
- 4.5：`zotero_find_items` 与 `zotero_search_items` 共享内部桥接执行路径（语义边界不变）。
- 4.7：PDG 侧强调版本透明化（`pdg_info` / `pdg://info`），不做 freshness 判定。
- 4.9：工具调用遥测为 opt-in（`HEP_ENABLE_TOOL_USAGE_TELEMETRY`），仅用于排程决策支持。
- 4.10：run-scoped 返回统一 `job` envelope（轮询 run manifest）。
- 4.11：语言策略为英文主语义 + 中文同步参考。

## I) PDG（离线数据库：`PDG_DB_PATH`）
