...
- Projects declare where their truth lives with `nullius init --mode=<engine|file>` (works on fresh and already-initialized roots): `engine` means the run/approve lifecycle drives the work; `file` means work is executed by hand or external runners, durable truth lives in the project files and dated run directories, and an idle `run_status` is normal (declared file-mode also drops the plan-focus reconnect warning). The declaration is descriptive, never enforcing — no command is disabled in either mode, and it can be re-declared at any time the same way. Human decisions made in conversation are recorded append-only via `nullius decision record` / `nullius decision pending` into `.nullius/decisions.jsonl` (read back with `nullius decision list`); the status receipt carries `execution_mode`, the decision ledger, and a diagnostic-only hint when an undeclared project looks file-mode in practice. That ledger is version-controlled, so its history is a branching graph and not one sequence: entry ids are therefore ULIDs minted per record (millisecond timestamp then 80 random bits, Crockford base32) rather than a counter derived from scanning the local file, which two branches would resolve to the same name — unique without coordination, and lexicographically ordered by recording time. Ids already colliding in a ledger are reported rather than tolerated: `decision list` exits non-zero and names the lines, the receipt carries `decision_ledger.duplicate_ids`, and `--resolves` refuses an id the file carries twice.
