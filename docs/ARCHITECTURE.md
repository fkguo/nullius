...
- Projects declare where their truth lives with `nullius init --mode=<engine|file>` (works on fresh and already-initialized roots): `engine` means the run/approve lifecycle drives the work; `file` means work is executed by hand or external runners, durable truth lives in the project files and dated run directories, and an idle `run_status` is normal (declared file-mode also drops the plan-focus reconnect warning). The declaration is descriptive, never enforcing — no command is disabled in either mode, and it can be re-declared at any time the same way. Human decisions made in conversation are recorded append-only via `nullius decision record` / `nullius decision pending` into `.nullius/decisions.jsonl` (read back with `nullius decision list`); the status receipt carries `execution_mode`, the decision ledger, and a diagnostic-only hint when an undeclared project looks file-mode in practice. That ledger is version-controlled, so its history is a branching graph and not one sequence: entry ids are therefore ULIDs minted per record (a millisecond timestamp then 80 random bits, Crockford base32) rather than a counter derived from scanning the local file, which gave two branches the same name every time. The new guarantee is probabilistic and stated as such — two branches collide only if the same millisecond and all 80 random bits coincide — and ids sort lexicographically by recording millisecond, two ids from within one millisecond being unordered. The residue is not left unattended: an id a ledger carries twice is reported rather than tolerated, with `decision list` exiting non-zero and naming the lines, `decision_ledger.duplicate_ids` in the receipt, and `--resolves` refusing that id. Numbers left over from the superseded `D<n>` counter are reported the same way (`decision_ledger.superseded_ids`), whether they sit in an entry's `id` or in a resolution still pointing at one: those lines are quarantined, so a question left open among them is no longer in the open count, and the receipt names them — with their line numbers and the reissue repair — instead of letting a migration look like an empty ledger.
