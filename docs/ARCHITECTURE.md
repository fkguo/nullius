...
- Projects declare where their truth lives with `nullius init --mode=<engine|file>` (works on fresh and already-initialized roots): `engine` means the run/approve lifecycle drives the work; `file` means work is executed by hand or external runners, durable truth lives in the project files and dated run directories, and an idle `run_status` is normal (declared file-mode also drops the plan-focus reconnect warning). The declaration is descriptive, never enforcing — no command is disabled in either mode. Human decisions made in conversation are recorded append-only via `nullius decision <record|pending|list>` into `.nullius/decisions.jsonl`; the status receipt carries `execution_mode`, the decision ledger, and a diagnostic-only hint when an undeclared project looks file-mode in practice.
