...
Unknown keys are rejected at every level by all three parsers (`review-swarm`, `derivation-verify`, `idea-pairwise-match`): the top level accepts only `version`/`families`/`policy` plus a `_notes` comment carrier, a family entry accepts only the four fields above, and `policy` accepts only its two documented fields. A typo in any field name (a misspelled `models`, say) is therefore a parse error everywhere, never a field silently treated as absent. Comments belong in top-level `_notes` (any JSON value, accepted and ignored) or in a family's `notes` string; no other extra keys are legal anywhere.
