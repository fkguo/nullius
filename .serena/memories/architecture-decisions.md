...
### [2026-07-10] Idea lifecycle is an enforced state machine; posterior writes derive the state

**Decision**:
- `idea_node_v1.lifecycle_state` is required (no validator-side default) with a seven-state machine: `candidate`, `admission_review`, `admitted`, `needs_refresh`, `admission_blocked`, `waiting_activation`, `archived`. `active` is retired; nodes are born `candidate` (seed import and `node.import_generated` alike). A new mutable `lifecycle_reason` records the most recent transition's reason; a stored value outside the machine fails loudly (`-32018` / `unknown_lifecycle_state`) with a migration hint instead of being defaulted.
- `node.set_lifecycle` enforces a transition table (illegal requests are rejected with the allowed next states in `error.data.details`) and entry preconditions on stored data: `candidate` requires posterior null, `needs_refresh` requires a posterior, `admitted` requires `posterior.status=current` plus scoring-eligible close-prior coverage. `admission_blocked` and `waiting_activation` both carry `activation_condition` (kind enum gains `required_evidence`); `archived` requires a non-empty reason (`-32002` / `archived_reason_required`). Self-transitions exist only for the two condition-carrying states (condition update). Revival from `archived` is re-intake (`candidate` / `needs_refresh`), never a shortcut back to `admitted`.
- `node.set_posterior` is legal only in `admission_review` / `admitted` / `needs_refresh` (`-32018` / `posterior_write_lifecycle_invalid`; a candidate must declare `admission_review` first so admission has a logged start), rejects a `current`-labeled posterior that its coverage cannot support (`-32002` / `posterior_status_not_supported_by_coverage`), and derives the lifecycle itself after the write (`current` → `admitted`, else `needs_refresh`; when the derivation changes the state it stamps `lifecycle_reason=posterior_status=<status>`), so lifecycle and stored posterior cannot drift.
- `rank.compute` ranks only `admitted` nodes; every other node is skipped with its lifecycle state as the reason, and admitted nodes are re-checked against stored data (`no_posterior` / `metadata_only` / `coverage_incomplete` / `posterior_not_current`) as defense in depth for hand-migrated stores. `node.promote` requires `admitted` (`promotion_blocked` reason renamed `node_not_admitted`). New error code `-32018` `lifecycle_transition_invalid`; `idea-mcp` maps it to invalid-params.
- Report layers must not hand-maintain node states: triage scores stay outside the store (a triage state is deliberately NOT part of the machine — its entry condition is invisible to the store), and per-node obstruction narratives belong to reports, keyed off `lifecycle_state` + `lifecycle_reason` + `activation_condition` instead of literal node-id lists.
- Migration for pre-machine stores is a one-time offline `nodes_latest.json` rewrite (documented in the engine README): map `active`/absent by data (no posterior → `candidate`; current posterior + eligible coverage → `admitted`; else `needs_refresh`), then apply judgment states via RPC.

**Why**: The external idea-campaign testbed had to hand-build exactly this machine in a report script (16 script versions, node states patched by literal node-id matching) because the engine's flat three-state lifecycle could not distinguish "has a posterior number" from "is allocation-ready" from "blocked on named missing evidence". Enforcing the transitions and data preconditions engine-side removes that whole class of report-layer state bookkeeping, catches honest process mistakes (skipping admission, comparing triage scores with formal posteriors), and keeps the store the single authority on where every idea stands.
