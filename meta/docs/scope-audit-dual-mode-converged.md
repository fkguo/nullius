...
| Q2: W_compute skills 扩展 | Skills 模式正确，MCP 工具化 | Skills 正确，**分阶段** MCP 化 | Skills 正确，**立即** MCP 化 | **Skills 模式 + 分阶段 MCP** (安全门禁先行) |
...
**物理学家写的代码**（FeynCalc `.wl`、Julia `.jl`、SymPy `.py` 计算脚本）通过 hep-calc skill 和 W_compute 以 subprocess 方式运行，与 idea-core 的实现语言完全无关。
...
## Q2: W_compute + Skills 扩展 — 收敛: Skills 模式正确 + 分阶段 MCP 化
...
Lean4 作为 W_compute 的一个无状态验证节点，完美适配 skills 模式:
...
**收敛裁决**: Codex 的安全关切是正确的。W_compute 执行任意 `subprocess.run(argv)` — 如果不加防护就暴露为 MCP 工具，等于给 LLM agent 一个任意命令执行接口。
...
Codex 建议计算子图用 W_compute run-card (approach c)，整体工作流用声明式 schema (approach b)。这是合理的分层:
- 研究生命周期图: workflow schema (节点 = 工具调用 / skill / run-card)
- 计算子图: W_compute run-card (节点 = 计算阶段)
...
1. **Workflow Schema 格式 (3/3)**: `research_workflow_v1.schema.json` — 声明式工作流图 + hash-in-ledger + 模板系统。定义统一状态模型，桥接 idea-core / W_compute / hep_run_writing 三个支柱。

2. **Computation Contract (Codex)**: UX-02 应该是一个**契约** (不仅是目录布局)，能编译为 W_compute run-cards 和/或 skill jobs，带有显式的 acceptance checks 和 expected outputs。
...
| **NEW-COMP-01** | Phase 2 late | W_compute MCP 工具表面设计 + 安全模型 (C-02 containment + A3 gating) | ~200 (设计) | C-02, NEW-R15-impl | 3/3 |
| **NEW-COMP-02** | Phase 3 | W_compute MCP 实现 (`compute_run_card_v2` / `compute_status` / `compute_resolve_gate`) | ~500 | NEW-COMP-01 | 3/3 |
...
1. **Compute 安全 (Codex, 3/3 认同)**: W_compute 执行 `subprocess.run(argv)` (`w_compute.py:551`)。MCP 暴露前必须有 C-02 级别的命令/输出验证 + 默认 A3 gating。这是 NEW-COMP-02 的硬性前置。
...
3. **统一状态模型 (Gemini)**: idea-core / W_compute / hep_run_writing 三个支柱目前用不同状态表示。如果不统一，LLM 无法连接端到端 pipeline。NEW-WF-01 (workflow schema) 应解决此问题。
