...
export function zodToMcpInputSchema(schema: z.ZodType<any, any>): Record<string, unknown> {
  ...
  // Some Zod constructs (e.g. discriminated unions / optional defaults) may
  // produce a schema without a top-level `type`. Clients like Cursor validate
  // `inputSchema.type === "object"` and will hide tools if this is missing.
