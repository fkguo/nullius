...
import { aggregateSem10, normalizeGrouping, shufflePapers, type Sem10Actual, type Sem10Expected, type Sem10Input } from './sem10EvalSupport.js';

describe('eval: SEM-10 topic/method grouping', () => {
  it('beats the locked keyword-heavy baseline on semantic grouping cases', async () => {
    ...
    const improved = await runEvalSet<Sem10Input, Sem10Actual>(evalSet, {
      run: async input => {
        ...
        return { ...base, permutation_stability: stabilities.reduce((sum, value) => sum + value, 0) / stabilities.length };
      },
      judge: (expected, actual) => {
        const exp = expected as Sem10Expected;
        const pass = Object.keys(exp.topic_assignments).every(key => exp.topic_assignments[key] === actual.topic_assignments[key]) && Object.keys(exp.method_assignments).every(key => exp.method_assignments[key] === actual.method_assignments[key]);
        return { passed: pass, metrics: { passed: pass ? 1 : 0 } };
      },
      aggregate: aggregateSem10,
    });

    expect(improved.aggregateMetrics.grouping_f1_overall ?? 0).toBeGreaterThanOrEqual(0.85);
    expect(improved.aggregateMetrics.permutation_stability_overall ?? 0).toBeGreaterThanOrEqual(0.85);
    expect(improved.aggregateMetrics.hard_subset_f1 ?? 0).toBeGreaterThanOrEqual((lockedBaseline.metrics.hard_subset_f1 ?? 0) + 0.05);
    expect((improved.aggregateMetrics.grouping_f1_overall ?? 0) - (lockedBaseline.metrics.grouping_f1_overall ?? 0)).toBeGreaterThanOrEqual(0.05);
  });
  ...
  holdoutIt('holds grouping quality on the locked holdout', async () => {
    ...
    expect(report.aggregateMetrics.permutation_stability_overall ?? 0).toBeGreaterThanOrEqual(0.85);
  });
});
