...
export function augmentRunWithPdfSurface(runId: string): void {
  ...
  const dim = 256;
  const pdfItems = [
    { version: 1, evidence_id: 'pdf_page_1', run_id: runId, project_id: meta.project_id, type: 'pdf_page', locator: { kind: 'pdf', page: 1 }, text: 'PAGE_GOLD_001 FIGURE_GOLD Mass spectrum with the threshold cusp.', normalized_text: normalize('PAGE_GOLD_001 FIGURE_GOLD Mass spectrum with the threshold cusp.') },
    { version: 1, evidence_id: 'pdf_page_2', run_id: runId, project_id: meta.project_id, type: 'pdf_page', locator: { kind: 'pdf', page: 2 }, text: 'PAGE_GOLD_002 TABLE_GOLD Branching fractions for the benchmark channel. CHUNK_GOLD The benchmark signal is driven by threshold enhancement in the main channel.', normalized_text: normalize('PAGE_GOLD_002 TABLE_GOLD Branching fractions for the benchmark channel. CHUNK_GOLD The benchmark signal is driven by threshold enhancement in the main channel.') },
    { version: 1, evidence_id: 'pdf_region_table', run_id: runId, project_id: meta.project_id, type: 'pdf_region', locator: { kind: 'pdf', page: 2, bbox: { x0: 0.1, y0: 0.1, x1: 0.9, y1: 0.3 } }, text: 'TABLE_GOLD Branching fractions for the benchmark channel.', normalized_text: normalize('TABLE_GOLD Branching fractions for the benchmark channel.') },
    { version: 1, evidence_id: 'pdf_region_figure', run_id: runId, project_id: meta.project_id, type: 'pdf_region', locator: { kind: 'pdf', page: 1, bbox: { x0: 0.2, y0: 0.2, x1: 0.8, y1: 0.45 } }, text: 'FIGURE_GOLD Mass spectrum with the threshold cusp.', normalized_text: normalize('FIGURE_GOLD Mass spectrum with the threshold cusp.') },
  ] as const;
