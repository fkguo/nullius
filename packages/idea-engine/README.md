...
- `node.rewrite_provenance` is a scoped correction path for generated nodes whose recorded provenance violates the contract's documented semantics — currently only `novelty_delta.closest_prior`, which must reference the prior WORK (URI or survey ref key), never a campaign node id. The rewrite validates the corrected value against the node's own retrieval receipts, synchronizes the reserved novelty-delta claim when the current card still carries it, and appends every correction to the engine-owned `operator_trace.inputs.provenance_rewrites` history. If a reviewed card revision already withdrew that claim, the trace is corrected without reintroducing it. The archived generation pack keeps the original value, so the chain original → corrections → current stays auditable.
