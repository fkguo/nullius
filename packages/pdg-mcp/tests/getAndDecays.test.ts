...
function makeM4FixtureDb(): string {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pdg-m4-db-'));
  ...
  runSqlite(
    ...
    [
      ...
      // Base particle + mass PDGID (for pdg_get)
...
describe('pdg_get + pdg_get_decays (M4)', () => {
  it('pdg_get writes a JSON artifact and returns uri + summary', async () => {
    const dbPath = makeM4FixtureDb();
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pdg-m4-data-'));
    process.env.PDG_DB_PATH = dbPath;
    process.env.PDG_DATA_DIR = dataDir;

    const res = await handleToolCall('pdg_get', { pdgid: 'S043M', artifact_name: 'test_get.json' }, 'standard');
    expect(res.isError).not.toBe(true);

    const payload = JSON.parse(res.content[0]?.text ?? '{}') as any;
    expect(payload.uri).toBe('pdg://artifacts/test_get.json');
    expect(payload.summary?.pdgid).toBe('S043M');

    const artifactPath = path.join(dataDir, 'artifacts', 'test_get.json');
    expect(fs.existsSync(artifactPath)).toBe(true);
    const txt = fs.readFileSync(artifactPath, 'utf-8');
    expect(txt).toContain('S043M');
  });

  it('pdg_get_decays writes a JSONL artifact and returns uri + preview', async () => {
    const dbPath = makeM4FixtureDb();
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pdg-m4-data-'));
    process.env.PDG_DB_PATH = dbPath;
    process.env.PDG_DATA_DIR = dataDir;
    ...
    const res = await handleToolCall(
      'pdg_get_decays',
      { particle: { name: 'W' }, artifact_name: 'test_decays.jsonl', limit: 10, start: 0 },
      'standard'
    );
    expect(res.isError).not.toBe(true);

    const payload = JSON.parse(res.content[0]?.text ?? '{}') as any;
    expect(payload.uri).toBe('pdg://artifacts/test_decays.jsonl');
    expect(payload.summary?.decays).toBe(1);
    expect(payload.summary?.preview?.[0]?.pdgid).toBe('S043.1');

    const artifactPath = path.join(dataDir, 'artifacts', 'test_decays.jsonl');
    expect(fs.existsSync(artifactPath)).toBe(true);
    const lines = fs.readFileSync(artifactPath, 'utf-8').trim().split('\n');
    expect(lines.length).toBe(1);
    const obj = JSON.parse(lines[0] ?? '{}') as any;
    expect(obj.pdgid).toBe('S043.1');
    expect(obj.decay).toContain('->');
    expect(obj.branching?.display_text).toContain('%');
  });

  it('pdg_get_decays rejects unavailable editions', async () => {
    const dbPath = makeM4FixtureDb();
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pdg-m4-data-'));
    process.env.PDG_DB_PATH = dbPath;
    process.env.PDG_DATA_DIR = dataDir;
    ...
    const res = await handleToolCall(
      'pdg_get_decays',
      { particle: { name: 'W' }, edition: '1900', limit: 10, start: 0 },
      'standard'
    );
