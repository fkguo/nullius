...
export function resolvePathWithinParent(parentDir: string, userPath: string, what: string): string {
  const resolvedParent = path.resolve(parentDir);
...
export function assertSafePathSegment(segment: string, what: string): void {
  if (typeof segment !== 'string' || segment.length === 0) {
    throw unsafeFs(`${what} must be a safe path segment`, { segment });
  }
  if (segment.length > 200) {
    throw unsafeFs(`${what} must be a safe path segment`, { segment, reason: 'too_long' });
  }
  if (segment.includes('/') || segment.includes('\\')) {
    throw unsafeFs(`${what} must be a safe path segment`, { segment, reason: 'path_separator' });
  }
  if (segment === '.' || segment === '..' || segment.includes('..')) {
    throw unsafeFs(`${what} must be a safe path segment`, { segment, reason: 'unsafe_segment' });
  }
}
