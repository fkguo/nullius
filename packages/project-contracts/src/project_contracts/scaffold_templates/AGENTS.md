...
- In both modes, decisions made in conversation are recorded with `nullius decision record "<what was decided>"` (open questions with `nullius decision pending "<question>"`; close one with `nullius decision record "<answer>" --resolves <id>`); open items stay counted in every status receipt (the oldest ten itemized, the remainder via `nullius decision list`) until resolved. Newly recorded entries carry provisional six-character branch handles and locally avoid retained mappings. After branch entries have reached the authoritative trunk, run `nullius decision land` there to assign permanent `D<n>` ids and rewrite handle-valued resolutions; never land on an unmerged branch. Existing `D<n>` ledgers remain valid without migration. A repeated current id or non-one-to-one retained provisional mapping is a fail-closed ledger defect that must be repaired before resolving or landing. In file mode this ledger is the engine-visible record of conversational approvals.
