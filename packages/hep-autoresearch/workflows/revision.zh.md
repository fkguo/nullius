...
> 若你还没有可编译 `paper_root`，先走 [draft](draft.md)（从 Draft_Derivation/KB 生成草稿）。
...
入口：
- 直接 runner：`python3 scripts/run_revision.py --tag <TAG> --paper-root paper --tex-main main.tex --i-approve-paper-edits`
- Orchestrator：`python3 scripts/orchestrator.py run --run-id <TAG> --workflow-id revision`（默认触发 A4）
