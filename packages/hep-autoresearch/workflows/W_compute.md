# W_compute — Generic compute DAG (run_card v2)

Chinese version: `workflows/W_compute.zh.md`.
...
W_compute is intentionally **domain-agnostic**: any physics/domain logic must live in the project plugin (e.g. `examples/<project>/scripts/*`) and be invoked via shell backends.
...
- `--resume` (resume from `artifacts/runs/<run_id>/w_compute/` when run-card matches)
...
- `artifacts/runs/<RUN_ID>/w_compute/run_card.json` (normalized + parameter-resolved snapshot)
- `artifacts/runs/<RUN_ID>/w_compute/phase_state.json` (per-phase status + provenance pointers)

Recommended:
- `artifacts/runs/<RUN_ID>/w_compute/logs/<phase_id>/*.txt` (stdout/stderr snapshots)
- `artifacts/runs/<RUN_ID>/w_compute/workspace/` (copied phase I/O workspace for postmortem/debug)
...
- v2: first-class integration: W_compute workspace → MCP evidence (Outcome Gate) → research-writer.
