# computation — 通用计算 DAG（run_card v2）

英文版本：`workflows/computation.md`。
...
computation **不包含领域逻辑**：物理/领域实现必须下沉到项目插件（例如 `examples/<project>/scripts/*`），通过 shell backend 调用。
...
- `--resume`（在 run-card 匹配时从 `artifacts/runs/<run_id>/computation/` 续跑）
...
- `artifacts/runs/<RUN_ID>/computation/run_card.json`（规范化 + 参数已解析快照）
- `artifacts/runs/<RUN_ID>/computation/phase_state.json`（逐 phase 状态 + 溯源指针）

建议：
- `artifacts/runs/<RUN_ID>/computation/logs/<phase_id>/*.txt`（stdout/stderr 快照）
- `artifacts/runs/<RUN_ID>/computation/workspace/`（复制的 phase I/O workspace，便于排错）
...
- v2：一等集成：computation workspace → MCP evidence（Outcome Gate）→ research-writer。
