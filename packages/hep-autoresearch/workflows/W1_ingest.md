...
Chinese version: `workflows/W1_ingest.zh.md`.
...
- Reading note contains `Verification status:` (W1 allows `metadata-only`, but downstream workflows must upgrade when they depend on the paper).
...
- v2: structured LaTeX extraction (symbol table, key equation/result locators) as inputs for W2/W4.
