...
- reading note 必须包含 `Verification status:`（ingest 阶段允许 `metadata-only`；但后续 workflow 依赖该论文时必须升级）。
...
- v2：对 LaTeX 做结构化抽取（符号表、关键公式定位、结果候选列表），为 reproduce / derivation-check workflow 提供输入。
