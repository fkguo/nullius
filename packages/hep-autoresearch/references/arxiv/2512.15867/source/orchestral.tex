...
%\usepackage{minted}
\usepackage[frozencache,cachedir=_minted/]{minted} % For arXiv submission
\usepackage{fancyvrb}  % For better verbatim handling
\usepackage{xcolor}
\usepackage{xspace}
\usepackage{setspace}
\usepackage{enumitem}  % For customizing list spacing
\usepackage{etoolbox}  % For \AtBeginEnvironment
\usepackage{varwidth}  % For variable-width boxes
...
% Use this when refering to Orchestral AI
\newcommand{\orchestralai}{\textsc{Orchestral AI}\xspace}
...
% Role colors
\definecolor{OrcUserColor}{RGB}{210, 80, 80}         % Pale Red
\definecolor{OrcAgentColor}{RGB}{120, 120, 255}     % Pale Blue
\definecolor{OrcSystemColor}{RGB}{180, 30, 180}      % Purple
...
% Multi-line tool message (for tool calls with long arguments that need wrapping)
% Takes one argument: the tool title (can include \\ for line breaks and \hphantom for alignment)
\newtcolorbox{orchestraltoolmessageml}[1]{%
  enhanced,
  breakable,
  colback=white,
  colframe=OrcToolColor,
  ...
  coltitle=OrcToolColor,
  fonttitle=\sffamily\orchestralfontsize,
  title={\begin{varwidth}{\dimexpr\linewidth-\orchestralnestindent-1em-3.5mm\relax}#1\end{varwidth}},
}
