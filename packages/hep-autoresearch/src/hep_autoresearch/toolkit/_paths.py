...
def manifest_cwd(*, repo_root: Path, cwd: Path | None = None) -> str:
    ...
    Override: set `HEP_RESEARCH_RECORD_ABS_PATHS=1` to record absolute paths.
    """
    record_abs = os.environ.get("HEP_RESEARCH_RECORD_ABS_PATHS", "").strip().lower() in {"1", "true", "yes", "y"}
    if record_abs:
