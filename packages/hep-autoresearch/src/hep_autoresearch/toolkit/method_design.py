...
def method_design_one(inps: MethodDesignInputs, *, repo_root: Path) -> dict[str, Any]:
    ...
    if template == "minimal_ok":
        ...
        _write_text(
            readme_path,
            (
                f"# {project_id} (Generated by Autoresearch method-design toolkit)\n\n"
                f"- template: `{template}`\n"
                f"- tag: `{inps.tag}`\n\n"
                "The generated run-card was validated during scaffold generation.\n\n"
                "## Run\n\n"
                "```bash\n"
                "autoresearch run \\\n"
                f"  --run-id {inps.tag} \\\n"
    ...
    manifest: dict[str, Any] = {
        "schema_version": 1,
        "created_at": created_at,
        "command": "autoresearch-internal method_design",
