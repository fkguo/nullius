...
def computation_one(inps: ComputationInputs, repo_root: Path) -> dict[str, Any]:
    """Execute a declarative run_card v2 DAG and write SSOT artifacts under artifacts/runs/<tag>/computation/."""
    ...
    workspace_dir = (repo_root / "artifacts" / "runs" / tag / "computation").resolve()
    ...
    summary: dict[str, Any] = {
        "schema_version": 1,
        "created_at": created_at,
        "definitions": {"workflow": "computation", "kind": "computation"},
        "stats": {
            "status": status,
