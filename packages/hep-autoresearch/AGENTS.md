...
- Do not run broad deletions (`rm -rf`, `git rm -r`) directly on top-level project paths.
...
1) Read `PROJECT_MAP.md` + `PROJECT_CHARTER.md` + this `AGENTS.md` (do not rely on memory).
...
- Keep `Draft_Derivation.md` Capsule current for each milestone/tag.
...
- For **every development milestone** (any code/design change that you plan to commit), run an **Opus + Gemini dual review** and archive the outputs under `artifacts/` before committing:
  - `python3 scripts/run_dual_review.py --tag <TAG> --note "<what changed + why (project-level)>"`  # exits nonzero if NOT_READY
  - If NOT_READY: fix, rerun, and only then commit (or write an explicit adjudication note and record why you are overriding).
- Maintain a References section in `Draft_Derivation.md` and keep it updated; include links when available.
...
- Maintain a literature coverage matrix in `PREWORK.md` and update it as the project evolves (no blind spots across theory/method/numerics/baselines/data/closest prior work).
- Fill and maintain `PREWORK.md` `## Problem Framing Snapshot` (Problem Interpretation + P/D separation + sequential review checklist). This is a hard gate in theory_numerics/mixed/methodology_dev/toolkit_extraction profiles.
- If coverage is thin in any dimension, the team leader expands literature and records it in `knowledge_base/` and `PREWORK.md`. If skipped, add a one-line rationale in `PREWORK.md`.
- Before complex numerics or theory derivations, list candidate methods/algorithms and record the chosen approach (with rationale) in `PREWORK.md`.
...
- If lowering `capsule.min_headline_numbers` or using a per-capsule `Min headline numbers` override, record the rationale and compensating evidence in `PREWORK.md`.
...
- Numerics language preference (not enforced): if Julia is available, prefer Julia for new/heavy numerics (loops/sweeps/stability audits). Python is acceptable for small-scale calculations or when leveraging SciPy, but record the rationale in PREWORK/methodology trace and avoid pure-Python slow loops.
...
- Literature coverage matrix: fill dimensions/status/gaps in `PREWORK.md` and update during the project.
...
- Update Capsule I) in `Draft_Derivation.md` with all paths.
- Method selection checkpoint: record candidate methods and the chosen approach in `PREWORK.md`.
...
- Auto-fill reads `INITIAL_INSTRUCTION.md` and `PREWORK.md`.
...
- Notebook integrity gate (if enabled) fails when Draft_Derivation.md has duplicate marker blocks (REVIEW_EXCERPT/AUDIT_SLICES/Capsule) or violates math formatting rules.
...
- In Markdown tables, do NOT use literal `|` inside `$...$` (it often breaks table parsing). Use `\\lvert ... \\rvert` (or `\\lVert ... \\rVert`) instead.
...
- Avoid accidental double-backslash LaTeX escapes (common TOC/LLM artifact), e.g. `\\Delta`, `\\gamma\\_{\\rm lin}`. If they appear:
  - Find (deterministic targets): `bash ~/.codex/skills/research-team/scripts/bin/check_md_double_backslash.sh --notes Draft_Derivation.md [--fail]`
  - Fix (math regions only): `python3 ~/.codex/skills/research-team/scripts/bin/fix_markdown_double_backslash_math.py --notes Draft_Derivation.md --in-place`

## References (required)

- `Draft_Derivation.md` must include a `## References` section near the end.
