...
class TestOrchestratorStatusW3Reconcile(unittest.TestCase):
    ...
    def _write_state(self, repo_root: Path, *, run_id: str) -> None:
        ...
        state = {
            ...
            "plan": {
                ...
                "steps": [
                    {"step_id": "W3.PAPER_REVISER.A", "status": "in_progress", "description": "A"},
                    {"step_id": "W3.PAPER_REVISER.B", "status": "pending", "description": "B"},
                    {"step_id": "W3.PAPER_REVISER.C", "status": "pending", "description": "C"},
                    {"step_id": "W3.PAPER_REVISER.D", "status": "pending", "description": "D"},
                    {"step_id": "W3.PAPER_REVISER.E", "status": "pending", "description": "E"},
    ...
    def test_case_a_reconcile_completed(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(((data.get("w3_substeps") or {}).get("source")), "manifest")
    ...
    def test_case_b_manifest_partial_in_progress(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(((data.get("w3_substeps") or {}).get("source")), "manifest")
            statuses = ((data.get("w3_substeps") or {}).get("statuses")) or {}
    ...
    def test_case_c_manifest_missing_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(rc, 0)
            data = json.loads(out)
            self.assertEqual(((data.get("w3_substeps") or {}).get("source")), "state")
            codes = [str(w.get("code")) for w in (data.get("warnings") or []) if isinstance(w, dict)]
            self.assertIn("w3_manifest_missing", codes)
    ...
    def test_case_d_manifest_steps_malformed_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(((data.get("w3_substeps") or {}).get("source")), "state")
            codes = [str(w.get("code")) for w in (data.get("warnings") or []) if isinstance(w, dict)]
            self.assertIn("w3_manifest_steps_schema_invalid", codes)
