...
- `init`：初始化当前目录为 project root（补齐 docs/KB/specs 最小骨架；创建 `.autopilot/` 状态 + ledger）
...
- `reject <approval_id>`：拒绝并要求改计划/改预算
- `logs`：输出最近日志与关键失败点
...
用户回复 `approve/reject` 前不得继续执行该动作。
...
推荐保存位置（项目内）：
- `.autopilot/state.json`（选定：隐藏状态；与用户可读的 `team/` 日志分离）
