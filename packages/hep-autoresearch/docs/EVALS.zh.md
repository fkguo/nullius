...
- Workflow 级：W1/W2/W3/W4 是否稳定产出、是否通过门禁。
...
先做小而硬的 eval：
- E1/E2：W1 ingestion（固定论文输入，检查 reading-note 字段/链接/RefKey 完整性）
- E3：Orchestrator CLI presence（先把 `status/pause/resume/approve` 的代码与契约纳入回归集合）
- E4：W2 toy reproduction（确定性 toy 算例，检查 artifacts + 数值容差）
- E5：W3 改稿闭环（固定一个最小 LaTeX 工程，检查能否“审→改→编译” + provenance 产物）
- E6：Orchestrator 行为回归（W2/W3 默认门禁：exit code、审批包、产物路径断言）
...
[
  {
    "path": "references/inspire/recid-3112995/extracted.json",
    "pointer": "#/title",
    "type": "string",
    "contains": "MadAgents"
  },
  {
    "path": "artifacts/runs/M18-w1-failall-r1/ingest/recid-3112995-madagants/analysis.json",
    "pointer": "#/results/ok",
    "type": "boolean",
    "equals": false
  }
...
```bash
python3 scripts/run_evals.py --tag M1-eval-r1 --case-id E1-ingest-recid-3112995
```
