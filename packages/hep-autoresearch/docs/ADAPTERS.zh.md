...
说明：
- `required_gates` 是 adapter 的默认安全机制：任何会触发 network / 写代码 / 改稿 / 跑算力 的动作必须经由 A1–A5（除非 run-card 显式放宽）。
...
- `required_gates`：通常至少包含 `A3`（compute），若会写代码/改稿则加 `A2/A4`
...
- `required_gates` 通常至少包含 `A1`（mass_search / network）与 `A3`（compute）。
...
- approvals：adapter 在 `prepare()` 阶段声明 `required_gates`，由 Orchestrator 负责 `approve/reject` 状态机。
