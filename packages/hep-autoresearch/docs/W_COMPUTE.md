# W_compute user guide (run_card v2)

Chinese version: [docs/W_COMPUTE.zh.md](W_COMPUTE.zh.md).

W_compute is a **domain-agnostic**, **declarative**, **auditable** compute DAG runner. All domain logic must live in a project plugin (typically under [examples/](../examples/)).

This doc focuses on:
- what a `run_card v2` is
- how W_compute resolves paths/parameters
- what artifacts you should expect
- how to validate/run/resume safely

For the workflow-level overview, see: [workflows/W_compute.md](../workflows/W_compute.md).
...
If any phase uses a shell backend, W_compute requires explicit trust:
...
W_compute always writes SSOT artifacts under:
...
Resume is fail-closed: if the run-card snapshot or phase outputs do not match the expected state, W_compute should refuse to resume and ask you to rerun cleanly.
