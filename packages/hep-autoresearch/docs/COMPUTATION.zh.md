...
- 下文里的 `run-card validate/render` 与 `python3 scripts/orchestrator.py run --run-card ...` 仍是待退役的 legacy Pipeline A authoring/execution surface；这里保留它们，只是为了让已有的 run-card 示例仍然可读。
