...
生命周期说明：本文件覆盖当前仍可使用的 **Pipeline A** Python CLI surface（`hep-autoresearch` / `hepar`）。长期规划仍然是在 TS orchestrator 成为唯一编排器后将这组 surface 一起退役；在那之前，这些说明仍是过渡期 Python 路径的有效操作文档。
