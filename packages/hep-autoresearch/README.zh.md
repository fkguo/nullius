...
其中 `propose` / `skill-propose` 这类演进相关 wrapper 也不再经过 parser；若仍需 maintainer/eval 使用，应直接走脚本 / lower-level toolkit authority。
