  ...
  it('does not advertise a stale project-local launcher as a healthy status fallback', () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'if [ -e \'/private/tmp/deleted-worktree/packages/orchestrator/dist/cli.js\' ] && BANNER_OUT=$(\'/private/tmp/deleted-worktree/packages/orchestrator/dist/cli.js\' --launcher-protocol 2>/dev/null) && [ "$BANNER_OUT" = "nullius-launcher-protocol 2" ]; then',
        ...
        'if [ -n "$RESOLVED_NULLIUS" ] && [ "${RESOLVED_NULLIUS#/}" != "$RESOLVED_NULLIUS" ] && [ -f "$RESOLVED_NULLIUS" ] && [ ! "$RESOLVED_NULLIUS" -ef "$0" ] && BANNER_OUT=$("$RESOLVED_NULLIUS" --launcher-protocol 2>/dev/null) && [ "$BANNER_OUT" = "nullius-launcher-protocol 2" ]; then',
