...
import { getToolSpecs, getTools, handleToolCall, type ToolExposureMode, type ToolSpec } from '../tools/index.js';
import { zodToMcpInputSchema } from '../tools/mcpSchema.js';
...
describe('Tool registry contracts', () => {
  ...
  it('standard mode has fewer tools than full mode', () => {
    const standard = getToolSpecs('standard');
    const full = getToolSpecs('full');
    expect(full.length).toBeGreaterThanOrEqual(standard.length);
  });

  it('openalex_content is full-only', () => {
    const standard = getToolSpecs('standard');
    const full = getToolSpecs('full');
    expect(standard.some(s => s.name === 'openalex_content')).toBe(false);
    expect(full.some(s => s.name === 'openalex_content')).toBe(true);
  });
  ...
  it('negative control: contract test catches schema drift', () => {
    ...
    const defs = getTools('standard').map(d => {
      if (d.name !== 'openalex_search') return d;
      return { ...d, inputSchema: { type: 'object', properties: {}, required: [] } };
    });
