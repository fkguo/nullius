# W6-07 Review Packet (Round-001) — Residual-Audited Dispersion-Coupled SOCP + Cross-Solver Envelope
...
## Key numbers (must match `results.json`)

From the robustness summary:
...
## Cross-solver envelope (Clarabel vs ECOS)

ECOS returns a higher minimum at the same Q2 mini-set (notably at $Q^*$), while both solvers show small constraint-margin violations at the ~1e-7–1e-6 level.
The residual auditing makes this difference explicit and reviewable.
...
1) **Readiness**: Is “residual-audited v18 + cross-solver envelope” sufficient to call Phase K/L `VERDICT: READY`?
2) **Claim boundary**: Given the cross-solver spread, what is the correct way to state the bound (conservative min across solvers? include residual tolerances as systematic)?
3) **Robustness gates**: Should we enforce a hard threshold on residual margins (e.g. all min margins >= -1e-7) and fail the run otherwise?
4) **Next tightening ideas** (still pion-only; no coupled-channel): which new positivity theorem/constraint should be tried next (e.g. Gram PSD / completely-monotone moment constraints), and how should we prioritize?
