...
- 保留 ad-hoc / 不统一的旧实现"以免重写"——**重构期间遇到结构不合理的旧代码，直接用新设计重写**，不要为了"最小改动"而在新旧两套模式间妥协
...
格式建议：`phase{P}-batch{N}-review-system.md`、`phase{P}-batch{N}-review-r{M}.md`、`phase{P}-batch{N}-r{M}-review/`（swarm 输出目录）。`meta/docs/` 只存放设计文档，不存放 review 产物。实施 prompt 存放在 `meta/docs/prompts/`（命名：`prompt-phase{N}-impl-batch{M}.md`）。
...
python3 skills/review-swarm/scripts/bin/run_multi_task.py \
  --out-dir ~/.autoresearch-lab-dev/batch-reviews/phase{P}-batch{N}-r{M}-review \
  --system ~/.autoresearch-lab-dev/batch-reviews/phase{P}-batch{N}-review-system.md \
  --prompt ~/.autoresearch-lab-dev/batch-reviews/phase{P}-batch{N}-review-r{M}.md
```
...
This project is indexed by GitNexus as **autoresearch-lab** (12922 symbols, 27485 relationships, 300 execution flows).
