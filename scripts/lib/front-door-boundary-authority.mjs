  ...
  },
  {
    ...
    snippets: [
      ...
      'MCP env 里的路径是 filesystem roots，不是 URI/protocol 设置。',
	      ...
	      '`.autoresearch/HARNESS` 是 `autoresearch init` 写入的机器可读 runtime handshake',
	      'You are in a folder that should be managed by autoresearch.',
	      'If AGENTS.md and .autoresearch/HARNESS are both missing, initialize the project:',
	      'If `autoresearch` is not available, first prepare the CLI from the source checkout',
  ...
  },
  {
    ...
    snippets: [
      ...
      '## 2. Design Invariants',
	      ...
	      '- `autoresearch init` writes `.autoresearch/HARNESS` as the machine-readable project handshake.',
  ...
  },
  {
    ...
    snippets: [
      ...
      '### 0.0 先确认 front-door 角色',
	      ...
	      '- `.autoresearch/HARNESS` = `autoresearch init` 写入的机器可读 runtime handshake',
