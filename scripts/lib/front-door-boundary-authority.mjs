...
export const FRONT_DOOR_SNIPPETS = [
  ...
  },
  {
    ...
    snippets: [
      ...
      '原始检索摘要、下载尝试和控制面观察写入 `research_plan.md` progress log 或 `artifacts/runs/<TAG>/`',
  ...
  },
  {
    ...
    snippets: [
      ...
      'Keep dated run logs, raw workflow summaries, and tool-use traces in `research_plan.md` progress entries or `artifacts/runs/<TAG>/`',
  ...
  },
  {
    ...
    snippets: [
      ...
      '带日期的执行记录、原始 workflow 摘要和工具调用过程应放在 `research_plan.md` progress log 或 `artifacts/runs/<TAG>/`',
  ...
  },
  {
    ...
    snippets: [
      ...
      'dated execution traces, raw run observations, and tool-use traces belong in `research_plan.md` or `artifacts/runs/<TAG>/`',
    ],
    orderedSnippets: [
      '- **Main generic lifecycle + native TS computation + workflow-plan entrypoint**: `autoresearch` CLI for external project roots and `.autoresearch/` state',
      '- **Recommended public stateful literature workflow entrypoint**: `autoresearch workflow-plan` (requires an initialized external project root; resolves recipes directly via `@autoresearch/literature-workflows`; persists `.autoresearch/state.json#/plan` and derives `.autoresearch/plan.md`)',
  ...
  },
  {
    ...
    snippets: [
      ...
      'important literature notes require full-text/source-first reading, auditable section/page/equation/figure coverage, and LaTeX math notation',
      'raw dated run logs and tool-use traces belong in `research_plan.md` or `artifacts/runs/<TAG>/`',
