// # CONTRACT-EXEMPT: CODE-01.1 Pre-existing ordered cross-document public-truth registry; this integration updates only literal report-validation and idea-runtime assertions, while splitting unrelated authority entries would expand the bounded change and weaken atomic anti-drift checking.
...
export const FRONT_DOOR_SNIPPETS = [
  ...
  },
  {
    ...
    snippets: [
      ...
      'posterior 排序的 rank/promote、节点 posterior/lifecycle/grounding-audit/card-revision 更新、溯源修正与 generation-pack 导入（`node.import_generated`）属于 `idea-engine` runtime-contract truth，不是 root front door',
  ...
  },
  {
    ...
    snippets: [
      ...
      '## 2. Design Invariants',
	      ...
	      '- `nullius init` writes `.nullius/HARNESS` as the machine-readable project handshake and writes platform launchers without changing lifecycle authority:',
	      '`.nullius/bin/nullius.cmd`',
	      'process-tree containment',
