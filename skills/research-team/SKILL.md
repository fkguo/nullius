...
- **Refresh the contract's notebook-derived block**: `scripts/bin/refresh_research_contract.py --root /path/to/project`. Only the region between the notebook-sync markers is rewritten; everything the project owns in `research_contract.md` is left alone, and `nullius init` no longer touches the file at all once it exists.
  ...
  - Because of that, the refresh **never removes a line it did not just re-derive**. Existing lines the scan did not reproduce are written back verbatim under a heading that says so, and reported in `retained_entries`. A blind spot in the scanner therefore costs a stale line you can read and delete, not a bibliography. Nothing decides for you whether such a line was deleted on purpose — that judgement was tried twice and destroyed real references both times.
