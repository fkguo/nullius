...
- **Refresh the contract's notebook-derived block**: `scripts/bin/refresh_research_contract.py --root /path/to/project`. Only the region between the notebook-sync markers is rewritten; everything the project owns in `research_contract.md` is left alone, and `nullius init` no longer touches the file at all once it exists.
  - The section and reference lists come from a deterministic line scanner, not a Markdown parser. When the re-derived block would remove entries whose text is still in the notebook, the refresh **refuses and writes nothing**, naming them: that combination can only mean the scanner missed a shape it does not read (a table, running prose, a block quote, an HTML list), and overwriting would leave no copy. Edit the block by hand in that case.
  - Entries that are gone from the notebook too are a genuine deletion: those are written and reported under `removed_entries`.
  - `--allow-entry-loss` overrides the refusal when the removal really is intended.
