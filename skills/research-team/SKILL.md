...
- **Refresh the contract's notebook-derived block**: `scripts/bin/refresh_research_contract.py --root /path/to/project`. Only the region between the notebook-sync markers is rewritten; everything the project owns in `research_contract.md` is left alone, and `nullius init` no longer touches the file at all once it exists.
  - The section and reference lists come from a deterministic line scanner, not a Markdown parser: it reads `-`/`*`/`+` and numbered items, and does not see references written as a table, as running prose, inside a block quote, or as an HTML list.
  - Because of that, the refresh **removes a line from the block only when the new scan produced a line with exactly the same text** (or when you pass `--drop-unreproduced`). Everything else is written back verbatim under a heading that says so, and reported in `retained_entries`. The exceptions are the lines the tool itself writes — the source-notebook and checksum lines, its own sub-headings, and its placeholder lines — which are replaced each refresh; and a contract whose sync markers are out of order is refused rather than rewritten. A blind spot in the scanner therefore costs a stale line you can read and delete, not a bibliography. Nothing decides for you whether such a line was deleted on purpose — that judgement was tried twice and destroyed real references both times.
  - `--drop-unreproduced` is the one path that removes anything: use it after reading the retained list.
