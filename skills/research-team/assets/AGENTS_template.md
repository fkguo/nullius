...
- If the host agent exposes a `research-harness` skill or equivalent project-harness entrypoint, use that entrypoint first for reconnect, recovery, routing, verification, and handoff.
- `research-harness` restores durable project state, then routes lifecycle work to `autoresearch`, milestone execution to this `research-team` workflow, and provider/domain work to the relevant tool layer.
- Treat `research-team` as the milestone execution layer, not the root project control plane. Stable conclusions from `team/runs/` are incomplete until folded back into `research_contract.md`, `research_plan.md#Current Status`, and `artifacts/runs/<run_id>/`.
- If this AGENTS.md was generated by `research-team`, `research-harness`, `autoresearch init`, or another scaffold tool, preserve this project-harness precedence block and the `.autoresearch/HARNESS` trigger.
