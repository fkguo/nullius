...
- Generate `PROJECT_START_PROMPT.md` (kickoff prompt) and review/approve it before auto-run:
  - `python3 "${SKILL_DIR:-$(for r in "${CLAUDE_CONFIG_DIR:-$HOME/.claude}" "${CODEX_HOME:-$HOME/.codex}" "$HOME/.config/opencode"; do [ -d "$r/skills/research-team" ] && echo "$r/skills/research-team" && break; done)}/scripts/bin/generate_project_start_prompt.py" --root .`
