...
def main() -> int:
    ...
    if not agents.is_file():
        ...
        print('      scaffold: bash "${SKILL_DIR:-$(for r in "${CLAUDE_CONFIG_DIR:-$HOME/.claude}" "${CODEX_HOME:-$HOME/.codex}" "$HOME/.config/opencode"; do [ -d "$r/skills/research-team" ] && echo "$r/skills/research-team" && break; done)}/scripts/bin/scaffold_research_workflow.sh" --root <root> --project <name> --profile mixed')
        return 1
