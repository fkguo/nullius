...
- Preserve unresolved questions as explicit blockers rather than burying them in chat or transient team logs. When a question needs the project owner's decision, also record it with `nullius decision pending "<question>"` — it stays visible in every status receipt until a later `nullius decision record "<what was decided>" --resolves <id>` closes it, instead of scrolling away in conversation.
