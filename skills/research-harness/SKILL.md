...
- The result JSON is always printed on stdout; `--output` additionally persists it atomically. The `--output` write is guarded on every exit path: an `--output` that aliases any input the run consumed — the record, the plan, a verdict file, or the observed fingerprint, including case-insensitive-filesystem aliases — is never written and the run exits `2`; a failed `--output` write likewise makes the checker exit `2` even for an authorized verdict, because an authorization whose requested audit artifact cannot be persisted is refused.
