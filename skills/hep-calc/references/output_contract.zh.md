...
out_dir/
  ...
  symbolic/
    symbolic.json
    status.json
    input_bindings.json         # mathematica.bound_inputs 非空时
    bound_inputs/               # 已声明上游输入的新鲜快照
  numeric/
    numeric.json
    status.json
  tex/
...
- `data.assertions` 是可选的 fail-closed 断言列表。每项必须含非空字符串 `id` 和布尔值 `passed`，
  ...
  `symbolic/status.json.status = FAIL`，并令根级 `overall_status = FAIL`。若 `mathematica.bound_inputs` 非空，
  此列表为必需且必须至少包含一个有效 assertion；每个已声明输入还必须出现在 accessor 记录的
  `data.bound_inputs_consumed` 见证中。
