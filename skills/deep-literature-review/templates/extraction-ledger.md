# Extraction ledger

Use one item block per load-bearing equation, value, definition, quotation, or
attribution. Copy the block as needed. Do not combine literal transcription,
notation normalization, and derived interpretation in one field.

## Source identity

- Work identifier:
- Identity-triangulation artifact:
- Claimed series/erratum/companion relation and primary-source locator:
- Correction-chain search performed (erratum/corrigendum/retraction/revised version):
- Correction status and search evidence:
- Correction-search evidence path(s) and SHA-256:
- Correction identifiers and exact affected-item locators:
- Persisted correction source path(s) and SHA-256:
- Persisted source path:
- Source SHA-256:
- Source form:
- Source-text origin: `direct-original-text` / `visually-verified-transcription`
- Source-provenance evidence path and SHA-256:
- Original page/crop path and SHA-256 (PDF or scan):
- Excerpt-to-rendered-page visual check:
- Visual verifier distinct from transcriber:
- Literal source payload contains curator normalization or inference: no
- Reader:
- Candidate note visible during extraction: no
- Prior verdict or proposed correction visible during extraction: no
- Declared comparison scope: selected items / complete section / complete paper
- Formula-and-condition inventory path and SHA-256 (required for complete scope):
- Inventory completeness independently checked:

## Item 1

- Neutral question:
- Exact locator:
- Neighboring domain/boundary/branch/sheet/convention text required by this item:
- Dependency closure for the requested derivation:
- Literal source text:
- Printed form superseded by a correction: no
- Corrected source text and correction locator:
- Normalized transcription:
- Literal-to-normalized change log (including primes and dummy-variable renames):
- Token-class audit (signs, coefficients/factors, operators, indices/subscripts,
  relations/endpoints, numerator/denominator, exponents, primes, arguments,
  integration limits):
- Symbol dictionary:
- Derived mapping or inference:
- First ambiguity or divergence:
- Independent candidate-withheld extraction:
- Candidate-visible source comparison:
- Status: unverified

Allowed status values: `unverified`, `match`, `diverge`, `uncheckable`.

When feedback is needed, send only the neutral question, source locator, and
suspected divergence. Do not include the desired replacement as a premise.
